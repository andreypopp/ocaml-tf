...
type azurerm_hdinsight_kafka_cluster__rest_proxy = {
  ...
  security_group_name : string;  (** security_group_name *)
}
[@@deriving yojson_of]
...
type azurerm_hdinsight_kafka_cluster__roles__worker_node = {
  number_of_disks_per_node : float;  (** number_of_disks_per_node *)
  password : string option; [@option]  (** password *)
  ssh_keys : string list option; [@option]  (** ssh_keys *)
  subnet_id : string option; [@option]  (** subnet_id *)
...
let azurerm_hdinsight_kafka_cluster ?encryption_in_transit_enabled
    ?tags ?tls_min_version ?timeouts ~cluster_version ~location ~name
