...
let azurerm_cosmosdb_gremlin_database ?timeouts ~account_name ~name
    ~resource_group_name ~autoscale_settings __resource_id =
  let __resource_type = "azurerm_cosmosdb_gremlin_database" in
  let __resource =
    {
      account_name;
      name;
  ...
  Resource.add ~type_:__resource_type ~id:__resource_id
    (yojson_of_azurerm_cosmosdb_gremlin_database __resource);
  ()
