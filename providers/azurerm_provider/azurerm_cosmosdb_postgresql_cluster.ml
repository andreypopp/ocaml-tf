...
let azurerm_cosmosdb_postgresql_cluster ?administrator_login_password
    ...
    ?ha_enabled ?node_public_ip_access_enabled ?node_server_edition
    ?point_in_time_in_utc ?preferred_primary_zone ?source_location
    ?source_resource_id ?tags ?timeouts ~location ~name ~node_count
    ~resource_group_name ~maintenance_window __resource_id =
