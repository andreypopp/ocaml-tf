...
type cloudflare_load_balancer_pool__load_shedding = {
  default_percent : float option; [@option]
      (** Percent of traffic to shed 0 - 100. Defaults to `0`. *)
  default_policy : string option; [@option]
      (** Method of shedding traffic. Available values: ``, `hash`, `random`. Defaults to ``. *)
  session_percent : float option; [@option]
      (** Percent of session traffic to shed 0 - 100. Defaults to `0`. *)
  session_policy : string option; [@option]
...
type cloudflare_load_balancer_pool__origins__header = {
  ...
  values : string list;  (** Values for the HTTP headers. *)
}
[@@deriving yojson_of]
...
type cloudflare_load_balancer_pool = {
  ...
  minimum_origins : float option; [@option]
  ...
  monitor : string option; [@option]
      (** The ID of the Monitor to use for health checking origins within this pool. *)
  name : string;  (** A short name (tag) for the pool. *)
  notification_email : string option; [@option]
