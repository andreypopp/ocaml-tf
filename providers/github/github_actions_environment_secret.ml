...
let _ = fun (_ : github_actions_environment_secret) -> ()

let yojson_of_github_actions_environment_secret =
  (function
   | {
...
let _ = yojson_of_github_actions_environment_secret

[@@@deriving.end]

let github_actions_environment_secret ?encrypted_value ?id
    ?plaintext_value ~environment ~repository ~secret_name () :
    github_actions_environment_secret =
  {
    encrypted_value;
    environment;
...
let make ?encrypted_value ?id ?plaintext_value ~environment
    ~repository ~secret_name __id =
  let __type = "github_actions_environment_secret" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_actions_environment_secret
        (github_actions_environment_secret ?encrypted_value ?id
           ?plaintext_value ~environment ~repository ~secret_name ());
    attrs = __attrs;
  }

let register ?tf_module ?encrypted_value ?id ?plaintext_value
    ~environment ~repository ~secret_name __id =
  let (r : _ Tf_core.resource) =
    make ?encrypted_value ?id ?plaintext_value ~environment
      ~repository ~secret_name __id
  in
