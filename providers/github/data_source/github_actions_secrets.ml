...
let _ = fun (_ : github_actions_secrets) -> ()

let yojson_of_github_actions_secrets =
  (function
   | { full_name = v_full_name; id = v_id; name = v_name } ->
       ...
       in
       `Assoc bnds
    : github_actions_secrets -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_github_actions_secrets

[@@@deriving.end]

let github_actions_secrets ?full_name ?id ?name () :
...
let make ?full_name ?id ?name __id =
  let __type = "github_actions_secrets" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_actions_secrets
        (github_actions_secrets ?full_name ?id ?name ());
    attrs = __attrs;
  }
