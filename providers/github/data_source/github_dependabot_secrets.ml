...
let _ = fun (_ : github_dependabot_secrets) -> ()

let yojson_of_github_dependabot_secrets =
  (function
   | { full_name = v_full_name; id = v_id; name = v_name } ->
       ...
       in
       `Assoc bnds
    : github_dependabot_secrets -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_github_dependabot_secrets

[@@@deriving.end]

let github_dependabot_secrets ?full_name ?id ?name () :
...
let make ?full_name ?id ?name __id =
  let __type = "github_dependabot_secrets" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_dependabot_secrets
        (github_dependabot_secrets ?full_name ?id ?name ());
    attrs = __attrs;
  }
