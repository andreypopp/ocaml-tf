...
type github_codespaces_user_secrets = {
  id : string prop option; [@option]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : github_codespaces_user_secrets) -> ()

let yojson_of_github_codespaces_user_secrets =
  (function
   | { id = v_id } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
...
let _ = yojson_of_github_codespaces_user_secrets

[@@@deriving.end]

let github_codespaces_user_secrets ?id () :
    github_codespaces_user_secrets =
  { id }
...
let make ?id __id =
  let __type = "github_codespaces_user_secrets" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_codespaces_user_secrets
        (github_codespaces_user_secrets ?id ());
    attrs = __attrs;
  }
