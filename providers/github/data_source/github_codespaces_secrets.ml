...
let _ = fun (_ : github_codespaces_secrets) -> ()

let yojson_of_github_codespaces_secrets =
  (function
   | { full_name = v_full_name; id = v_id; name = v_name } ->
       ...
       in
       `Assoc bnds
    : github_codespaces_secrets -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_github_codespaces_secrets

[@@@deriving.end]

let github_codespaces_secrets ?full_name ?id ?name () :
...
let make ?full_name ?id ?name __id =
  let __type = "github_codespaces_secrets" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_codespaces_secrets
        (github_codespaces_secrets ?full_name ?id ?name ());
    attrs = __attrs;
  }
