...
let _ = fun (_ : github_codespaces_secret) -> ()

let yojson_of_github_codespaces_secret =
  (function
   | {
       ...
       in
       `Assoc bnds
    : github_codespaces_secret -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_github_codespaces_secret

[@@@deriving.end]

let github_codespaces_secret ?encrypted_value ?id ?plaintext_value
    ~repository ~secret_name () : github_codespaces_secret =
...
let make ?encrypted_value ?id ?plaintext_value ~repository
    ~secret_name __id =
  let __type = "github_codespaces_secret" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_codespaces_secret
        (github_codespaces_secret ?encrypted_value ?id
