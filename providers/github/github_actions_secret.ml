...
let _ = fun (_ : github_actions_secret) -> ()

let yojson_of_github_actions_secret =
  (function
   | {
       ...
       in
       `Assoc bnds
    : github_actions_secret -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_github_actions_secret

[@@@deriving.end]

let github_actions_secret ?encrypted_value ?id ?plaintext_value
    ~repository ~secret_name () : github_actions_secret =
...
let make ?encrypted_value ?id ?plaintext_value ~repository
    ~secret_name __id =
  let __type = "github_actions_secret" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_actions_secret
        (github_actions_secret ?encrypted_value ?id ?plaintext_value
