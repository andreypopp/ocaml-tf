...
type conditions__repository_name = {
  ...
  include_ : string prop list;
      ...
      [@yojson_drop_default Stdlib.( = )]
      [@key "include"]
  protected : bool prop option; [@option]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : conditions__repository_name) -> ()

let yojson_of_conditions__repository_name =
  (function
   | {
       exclude = v_exclude;
       include_ = v_include_;
...
let _ = yojson_of_conditions__repository_name

[@@@deriving.end]

type conditions = {
  repository_id : float prop list option; [@option]
...
let yojson_of_conditions =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_repository_name then bnds
         else
           let arg =
             (yojson_of_list yojson_of_conditions__repository_name)
               v_repository_name
           in
       ...
       in
       let bnds =
         match v_repository_id with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg =
               yojson_of_list (yojson_of_prop yojson_of_float) v
             in
             let bnd = "repository_id", arg in
             bnd :: bnds
       in
       `Assoc bnds
...
let _ = fun (_ : rules__required_workflows__required_workflow) -> ()

let yojson_of_rules__required_workflows__required_workflow =
  (function
   | { path = v_path; ref = v_ref; repository_id = v_repository_id }
     ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_float v_repository_id in
...
let _ = yojson_of_rules__required_workflows__required_workflow

[@@@deriving.end]

type rules__required_workflows = {
  required_workflow :
    rules__required_workflows__required_workflow list;
      [@default []] [@yojson_drop_default Stdlib.( = )]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : rules__required_workflows) -> ()

let yojson_of_rules__required_workflows =
  (function
   | { required_workflow = v_required_workflow } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         if Stdlib.( = ) [] v_required_workflow then bnds
         else
           let arg =
             (yojson_of_list
                yojson_of_rules__required_workflows__required_workflow)
               v_required_workflow
           in
           let bnd = "required_workflow", arg in
           bnd :: bnds
       in
       `Assoc bnds
    : rules__required_workflows -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_rules__required_workflows
...
let yojson_of_rules =
  (function
   | {
       ...
       required_workflows = v_required_workflows;
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_required_workflows then bnds
         else
           let arg =
             (yojson_of_list yojson_of_rules__required_workflows)
               v_required_workflows
           in
           let bnd = "required_workflows", arg in
           bnd :: bnds
       in
       let bnds =
...
let _ = fun (_ : github_organization_ruleset) -> ()

let yojson_of_github_organization_ruleset =
  (function
   | {
...
let _ = yojson_of_github_organization_ruleset
...
let conditions__repository_name ?protected ~exclude ~include_ () :
    conditions__repository_name =
  { exclude; include_; protected }

let conditions ?repository_id ?(repository_name = []) ~ref_name () :
    conditions =
  { repository_id; ref_name; repository_name }
...
let rules__required_workflows__required_workflow ?ref ~path
    ~repository_id () : rules__required_workflows__required_workflow
    =
  { path; ref; repository_id }

let rules__required_workflows ~required_workflow () :
...
let rules ?creation ?deletion ?non_fast_forward
    ...
    ?(required_workflows = []) ?(tag_name_pattern = []) () : rules =
  {
    creation;
    deletion;
    non_fast_forward;
...
let github_organization_ruleset ?id ?(bypass_actors = [])
    ?(conditions = []) ~enforcement ~name ~target ~rules () :
    github_organization_ruleset =
  { enforcement; id; name; target; bypass_actors; conditions; rules }
...
let make ?id ?(bypass_actors = []) ?(conditions = []) ~enforcement
    ~name ~target ~rules __id =
  let __type = "github_organization_ruleset" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_organization_ruleset
        (github_organization_ruleset ?id ~bypass_actors ~conditions
           ~enforcement ~name ~target ~rules ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?(bypass_actors = []) ?(conditions = [])
    ~enforcement ~name ~target ~rules __id =
  let (r : _ Tf_core.resource) =
    make ?id ~bypass_actors ~conditions ~enforcement ~name ~target
      ~rules __id
  in
