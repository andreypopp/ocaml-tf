...
let _ = fun (_ : github_repository_webhook) -> ()

let yojson_of_github_repository_webhook =
  (function
   | {
       active = v_active;
       events = v_events;
       ...
       in
       `Assoc bnds
    : github_repository_webhook -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_github_repository_webhook
...
let github_repository_webhook ?active ?id ?(configuration = [])
    ~events ~repository () : github_repository_webhook =
  { active; events; id; repository; configuration }
...
let make ?active ?id ?(configuration = []) ~events ~repository __id =
  let __type = "github_repository_webhook" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_repository_webhook
        (github_repository_webhook ?active ?id ~configuration ~events
           ~repository ());
    attrs = __attrs;
  }
...
let register ?tf_module ?active ?id ?(configuration = []) ~events
  ...
  let (r : _ Tf_core.resource) =
    make ?active ?id ~configuration ~events ~repository __id
  in
