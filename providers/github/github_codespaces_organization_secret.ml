...
type github_codespaces_organization_secret = {
...
let _ = fun (_ : github_codespaces_organization_secret) -> ()

let yojson_of_github_codespaces_organization_secret =
  (function
   | {
...
let _ = yojson_of_github_codespaces_organization_secret

[@@@deriving.end]

let github_codespaces_organization_secret ?encrypted_value ?id
    ?plaintext_value ?selected_repository_ids ~secret_name
    ~visibility () : github_codespaces_organization_secret =
  {
    encrypted_value;
    id;
    plaintext_value;
...
let make ?encrypted_value ?id ?plaintext_value
  ...
  let __type = "github_codespaces_organization_secret" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_codespaces_organization_secret
        (github_codespaces_organization_secret ?encrypted_value ?id
           ?plaintext_value ?selected_repository_ids ~secret_name
           ~visibility ());
    attrs = __attrs;
  }
