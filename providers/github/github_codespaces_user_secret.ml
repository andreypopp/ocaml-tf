...
type github_codespaces_user_secret = {
...
let _ = fun (_ : github_codespaces_user_secret) -> ()

let yojson_of_github_codespaces_user_secret =
  (function
   | {
...
let _ = yojson_of_github_codespaces_user_secret

[@@@deriving.end]

let github_codespaces_user_secret ?encrypted_value ?id
...
let make ?encrypted_value ?id ?plaintext_value
    ?selected_repository_ids ~secret_name __id =
  let __type = "github_codespaces_user_secret" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_github_codespaces_user_secret
        (github_codespaces_user_secret ?encrypted_value ?id
           ?plaintext_value ?selected_repository_ids ~secret_name ());
    attrs = __attrs;
  }
