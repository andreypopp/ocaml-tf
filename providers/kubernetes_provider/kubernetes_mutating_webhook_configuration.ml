...
type kubernetes_mutating_webhook_configuration__metadata = {
...
type kubernetes_mutating_webhook_configuration__webhook__client_config__service = {
...
type kubernetes_mutating_webhook_configuration__webhook__client_config = {
...
Attempting to use a user or basic auth e.g. user:password@ is not allowed. Fragments (#...) and query parameters (?...) are not allowed, either. *)
  service :
    kubernetes_mutating_webhook_configuration__webhook__client_config__service
    list;
}
[@@deriving yojson_of]
(** ClientConfig defines how to communicate with the hook. Required *)

type kubernetes_mutating_webhook_configuration__webhook__namespace_selector__match_expressions = {
...
type kubernetes_mutating_webhook_configuration__webhook__namespace_selector = {
  match_labels : (string * string) list option; [@option]
      (** A map of {key,value} pairs. A single {key,value} in the matchLabels map is equivalent to an element of `match_expressions`, whose key field is key, the operator is In, and the values array contains only value. The requirements are ANDed. *)
  match_expressions :
    kubernetes_mutating_webhook_configuration__webhook__namespace_selector__match_expressions
    list;
}
[@@deriving yojson_of]
...
type kubernetes_mutating_webhook_configuration__webhook__object_selector__match_expressions = {
  key : string option; [@option]
      (** The label key that the selector applies to. *)
  operator : string option; [@option]
      (** A key's relationship to a set of values. Valid operators ard `In`, `NotIn`, `Exists` and `DoesNotExist`. *)
...
type kubernetes_mutating_webhook_configuration__webhook__object_selector = {
  match_labels : (string * string) list option; [@option]
      (** A map of {key,value} pairs. A single {key,value} in the matchLabels map is equivalent to an element of `match_expressions`, whose key field is key, the operator is In, and the values array contains only value. The requirements are ANDed. *)
  match_expressions :
    kubernetes_mutating_webhook_configuration__webhook__object_selector__match_expressions
    list;
}
[@@deriving yojson_of]
(** ObjectSelector decides whether to run the webhook based on if the object has matching labels. objectSelector is evaluated against both the oldObject and newObject that would be sent to the webhook, and is considered to match if either object matches the selector. A null object (oldObject in the case of create, or newObject in the case of delete) or an object that cannot have labels (like a DeploymentRollback or a PodProxyOptions object) is not considered to match. Use the object selector only if the webhook is opt-in, because end users may skip the admission webhook by setting the labels. Default to the empty LabelSelector, which matches everything. *)

type kubernetes_mutating_webhook_configuration__webhook__rule = {
...
type kubernetes_mutating_webhook_configuration__webhook = {
...
Defaults to Never. *)
  ...
  client_config :
    kubernetes_mutating_webhook_configuration__webhook__client_config
    list;
  namespace_selector :
    kubernetes_mutating_webhook_configuration__webhook__namespace_selector
    list;
  object_selector :
    kubernetes_mutating_webhook_configuration__webhook__object_selector
    list;
  rule :
    kubernetes_mutating_webhook_configuration__webhook__rule list;
}
[@@deriving yojson_of]
