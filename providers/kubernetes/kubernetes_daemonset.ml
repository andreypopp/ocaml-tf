...
let _ = fun (_ : kubernetes_daemonset) -> ()

let yojson_of_kubernetes_daemonset =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : kubernetes_daemonset -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_daemonset
...
let make ?id ?wait_for_rollout ?timeouts ~metadata ~spec __id =
  let __type = "kubernetes_daemonset" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_daemonset
        (kubernetes_daemonset ?id ?wait_for_rollout ?timeouts
           ~metadata ~spec ());
    attrs = __attrs;
  }
