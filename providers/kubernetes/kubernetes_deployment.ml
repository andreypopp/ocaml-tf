...
let _ = fun (_ : kubernetes_deployment) -> ()

let yojson_of_kubernetes_deployment =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : kubernetes_deployment -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_deployment
...
let make ?id ?wait_for_rollout ?timeouts ~metadata ~spec __id =
  let __type = "kubernetes_deployment" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_deployment
        (kubernetes_deployment ?id ?wait_for_rollout ?timeouts
           ~metadata ~spec ());
    attrs = __attrs;
  }
