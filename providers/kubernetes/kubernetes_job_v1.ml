...
let _ = fun (_ : kubernetes_job_v1) -> ()

let yojson_of_kubernetes_job_v1 =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : kubernetes_job_v1 -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_job_v1
...
let make ?id ?wait_for_completion ?timeouts ~metadata ~spec __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_job_v1
        (kubernetes_job_v1 ?id ?wait_for_completion ?timeouts
           ~metadata ~spec ());
    attrs = __attrs;
  }
