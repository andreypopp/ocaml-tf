...
let _ = fun (_ : kubernetes_daemon_set_v1) -> ()

let yojson_of_kubernetes_daemon_set_v1 =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : kubernetes_daemon_set_v1 -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_daemon_set_v1
...
let spec__strategy__rolling_update ?max_unavailable () :
    spec__strategy__rolling_update =
  { max_unavailable }
...
let spec ?min_ready_seconds ?revision_history_limit ?(selector = [])
...
let kubernetes_daemon_set_v1 ?id ?wait_for_rollout ?timeouts
    ~metadata ~spec () : kubernetes_daemon_set_v1 =
...
let make ?id ?wait_for_rollout ?timeouts ~metadata ~spec __id =
  let __type = "kubernetes_daemon_set_v1" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_daemon_set_v1
        (kubernetes_daemon_set_v1 ?id ?wait_for_rollout ?timeouts
           ~metadata ~spec ());
    attrs = __attrs;
  }
