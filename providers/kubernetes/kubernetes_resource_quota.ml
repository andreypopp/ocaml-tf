...
type spec = {
  hard : string prop Tf_core.assoc option; [@option]
...
let _ = fun (_ : kubernetes_resource_quota) -> ()

let yojson_of_kubernetes_resource_quota =
  (function
   | {
       id = v_id;
       metadata = v_metadata;
       ...
       in
       `Assoc bnds
    : kubernetes_resource_quota -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_resource_quota
...
let kubernetes_resource_quota ?id ?(spec = []) ?timeouts ~metadata ()
...
let make ?id ?(spec = []) ?timeouts ~metadata __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_resource_quota
        (kubernetes_resource_quota ?id ~spec ?timeouts ~metadata ());
    attrs = __attrs;
  }
