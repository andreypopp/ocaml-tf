...
let kubernetes_network_policy_v1 ?id ~metadata ~spec () :
    kubernetes_network_policy_v1 =
  { id; metadata; spec }
...
let make ?id ~metadata ~spec __id =
  let __type = "kubernetes_network_policy_v1" in
  let __attrs = ({ id = Prop.computed __type __id "id" } : t) in
  {
    Tf_core.id = __id;
    type_ = __type;
    json =
      yojson_of_kubernetes_network_policy_v1
        (kubernetes_network_policy_v1 ?id ~metadata ~spec ());
    attrs = __attrs;
  }
