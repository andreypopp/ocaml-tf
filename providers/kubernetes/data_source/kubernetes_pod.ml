...
let yojson_of_spec__init_container__security_context__capabilities =
  (function
   | { add = v_add; drop = v_drop } ->
       ...
       let bnds =
         let arg =
           yojson_of_list (yojson_of_prop yojson_of_string) v_drop
         in
         ("drop", arg) :: bnds
       in
       let bnds =
...
let kubernetes_pod ?id ~metadata () : kubernetes_pod =
  { id; metadata }
...
let make ?id ~metadata __id =
  ...
  in
  {
    Tf_core.id = __id;
    type_ = __type;
    json = yojson_of_kubernetes_pod (kubernetes_pod ?id ~metadata ());
    attrs = __attrs;
  }
