...
type kubernetes_secret_v1 = {
  binary_data : string prop Tf_core.assoc option; [@option]
...
let _ = fun (_ : kubernetes_secret_v1) -> ()

let yojson_of_kubernetes_secret_v1 =
  (function
   | {
       ...
       in
       `Assoc bnds
    : kubernetes_secret_v1 -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_secret_v1
...
let kubernetes_secret_v1 ?binary_data ?id ~metadata () :
...
let make ?binary_data ?id ~metadata __id =
  let __type = "kubernetes_secret_v1" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_secret_v1
        (kubernetes_secret_v1 ?binary_data ?id ~metadata ());
    attrs = __attrs;
  }

let register ?tf_module ?binary_data ?id ~metadata __id =
  let (r : _ Tf_core.resource) =
    make ?binary_data ?id ~metadata __id
  in
