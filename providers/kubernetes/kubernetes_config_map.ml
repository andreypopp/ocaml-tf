...
let _ = fun (_ : kubernetes_config_map) -> ()

let yojson_of_kubernetes_config_map =
  (function
   | {
       ...
       in
       `Assoc bnds
    : kubernetes_config_map -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_config_map
...
let make ?binary_data ?data ?id ?immutable ~metadata __id =
  let __type = "kubernetes_config_map" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_config_map
        (kubernetes_config_map ?binary_data ?data ?id ?immutable
           ~metadata ());
    attrs = __attrs;
  }
