...
type spec__strategy__rolling_update = {
  max_surge : string prop option; [@option]
  max_unavailable : string prop option; [@option]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : spec__strategy__rolling_update) -> ()

let yojson_of_spec__strategy__rolling_update =
  (function
   | { max_surge = v_max_surge; max_unavailable = v_max_unavailable }
     ->
       ...
       in
       `Assoc bnds
    : spec__strategy__rolling_update ->
      Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_spec__strategy__rolling_update

[@@@deriving.end]

type spec__strategy = {
  type_ : string prop option; [@option] [@key "type"]
  rolling_update : spec__strategy__rolling_update list;
      [@default []] [@yojson_drop_default Stdlib.( = )]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : spec__strategy) -> ()

let yojson_of_spec__strategy =
  (function
   | { type_ = v_type_; rolling_update = v_rolling_update } ->
       ...
       let bnds =
         if Stdlib.( = ) [] v_rolling_update then bnds
           ...
           let arg =
             (yojson_of_list yojson_of_spec__strategy__rolling_update)
               v_rolling_update
           in
       ...
       in
       `Assoc bnds
    : spec__strategy -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_spec__strategy
...
let yojson_of_spec =
  (function
   | {
       ...
       paused = v_paused;
       progress_deadline_seconds = v_progress_deadline_seconds;
       replicas = v_replicas;
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_strategy then bnds
         else
           let arg =
             (yojson_of_list yojson_of_spec__strategy) v_strategy
           in
       ...
       in
       let bnds =
         match v_progress_deadline_seconds with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_float v in
             let bnd = "progress_deadline_seconds", arg in
             bnd :: bnds
       in
       let bnds =
         match v_paused with
...
let _ = fun (_ : kubernetes_deployment_v1) -> ()

let yojson_of_kubernetes_deployment_v1 =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : kubernetes_deployment_v1 -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_deployment_v1
...
let spec__strategy__rolling_update ?max_surge ?max_unavailable () :
    spec__strategy__rolling_update =
  { max_surge; max_unavailable }

let spec__strategy ?type_ ?(rolling_update = []) () : spec__strategy
    =
...
let spec ?min_ready_seconds ?paused ?progress_deadline_seconds
    ?replicas ?revision_history_limit ?(selector = [])
...
let kubernetes_deployment_v1 ?id ?wait_for_rollout ?timeouts
    ~metadata ~spec () : kubernetes_deployment_v1 =
...
let make ?id ?wait_for_rollout ?timeouts ~metadata ~spec __id =
  let __type = "kubernetes_deployment_v1" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_kubernetes_deployment_v1
        (kubernetes_deployment_v1 ?id ?wait_for_rollout ?timeouts
           ~metadata ~spec ());
    attrs = __attrs;
  }
