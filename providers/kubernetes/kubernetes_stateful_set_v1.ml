...
let spec__volume_claim_template__spec ?storage_class_name
    ?volume_mode ?volume_name ~access_modes ~resources ~selector () :
    spec__volume_claim_template__spec =
  {
    access_modes;
    storage_class_name;
...
let spec ?pod_management_policy ?replicas ?revision_history_limit
    ~service_name ~persistent_volume_claim_retention_policy ~selector
    ~template ~update_strategy ~volume_claim_template () : spec =
  {
