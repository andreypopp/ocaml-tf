...
let _ = fun (_ : kubernetes_endpoints) -> ()

let yojson_of_kubernetes_endpoints =
  (function
   | { id = v_id; metadata = v_metadata; subset = v_subset } ->
       ...
       in
       `Assoc bnds
    : kubernetes_endpoints -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_kubernetes_endpoints
...
let make ?id ~metadata ~subset __id =
  let __type = "kubernetes_endpoints" in
  let __attrs = ({ id = Prop.computed __type __id "id" } : t) in
  {
    Tf_core.id = __id;
    type_ = __type;
    json =
      yojson_of_kubernetes_endpoints
        (kubernetes_endpoints ?id ~metadata ~subset ());
    attrs = __attrs;
  }
