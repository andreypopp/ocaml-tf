...
let aws_neptune_cluster_parameter_group ?description ?tags ~family
    ~parameter __resource_id =
  let __resource_type = "aws_neptune_cluster_parameter_group" in
  let __resource = { description; family; tags; parameter } in
  Resource.add ~type_:__resource_type ~id:__resource_id
    (yojson_of_aws_neptune_cluster_parameter_group __resource);
  ()
