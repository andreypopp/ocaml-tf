...
type aws_quicksight_refresh_schedule__schedule = {
  ...
  start_after_date_time : string option; [@option]
...
let aws_quicksight_refresh_schedule ~data_set_id ~schedule_id
    ~schedule __resource_id =
  let __resource_type = "aws_quicksight_refresh_schedule" in
  let __resource = { data_set_id; schedule_id; schedule } in
