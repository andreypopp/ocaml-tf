...
let aws_quicksight_iam_policy_assignment ?policy_arn ~assignment_name
    ~assignment_status ~identities __resource_id =
  let __resource_type = "aws_quicksight_iam_policy_assignment" in
  let __resource =
    { assignment_name; assignment_status; policy_arn; identities }
  in
