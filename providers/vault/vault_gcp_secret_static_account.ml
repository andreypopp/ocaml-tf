...
let _ = fun (_ : vault_gcp_secret_static_account) -> ()

let yojson_of_vault_gcp_secret_static_account =
  (function
   | {
       backend = v_backend;
       id = v_id;
       ...
       static_account = v_static_account;
     ...
     in
     let bnds =
       let arg = yojson_of_prop yojson_of_string v_static_account in
       ("static_account", arg) :: bnds
     in
     let bnds =
     ...
     in
     `Assoc bnds
    : vault_gcp_secret_static_account -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_vault_gcp_secret_static_account

[@@@deriving.end]

let binding ~resource ~roles () = ({ resource; roles } : binding)

let vault_gcp_secret_static_account ?id ?namespace ?secret_type ?token_scopes ~backend ~service_account_email
  ~static_account ~binding () =
  ({ backend; id; namespace; secret_type; service_account_email; static_account; token_scopes; binding }
...
let make ?id ?namespace ?secret_type ?token_scopes ~backend ~service_account_email ~static_account ~binding __id =
  let __type = "vault_gcp_secret_static_account" in
  let __attrs =
    ({
       tf_name = __id;
       ...
       static_account = Prop.computed __type __id "static_account";
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_vault_gcp_secret_static_account
        (vault_gcp_secret_static_account ?id ?namespace ?secret_type ?token_scopes ~backend ~service_account_email
           ~static_account ~binding ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?namespace ?secret_type ?token_scopes ~backend ~service_account_email ~static_account
  ~binding __id =
  let (r : _ Tf_core.resource) =
    make ?id ?namespace ?secret_type ?token_scopes ~backend ~service_account_email ~static_account ~binding __id
  in
