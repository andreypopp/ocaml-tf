...
type authenticators = {
  cert : string prop Tf_core.assoc option; [@option]
  userpass : string prop Tf_core.assoc option; [@option]
}
...
let yojson_of_vault_pki_secret_backend_config_est =
  (function
   | {
     ...
     in
     let bnds =
       match v_label_to_path_policy with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = Tf_core.yojson_of_assoc (yojson_of_prop yojson_of_string) v in
         let bnd = "label_to_path_policy", arg in
         bnd :: bnds
     in
     let bnds =
       match v_id with
     ...
     in
     let bnds =
       match v_default_mount with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_bool v in
         let bnd = "default_mount", arg in
         bnd :: bnds
     in
     let bnds =
...
let authenticators ?cert ?userpass () = ({ cert; userpass } : authenticators)

let vault_pki_secret_backend_config_est ?audit_fields ?default_mount ?default_path_policy ?enable_sentinel_parsing
  ?enabled ?id ?label_to_path_policy ?namespace ?(authenticators = []) ~backend () =
  ({
     audit_fields;
     backend;
     default_mount;
...
let make ?audit_fields ?default_mount ?default_path_policy ?enable_sentinel_parsing ?enabled ?id ?label_to_path_policy
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_vault_pki_secret_backend_config_est
        (vault_pki_secret_backend_config_est ?audit_fields ?default_mount ?default_path_policy ?enable_sentinel_parsing
           ?enabled ?id ?label_to_path_policy ?namespace ~authenticators ~backend ());
    attrs = __attrs;
  }

let register ?tf_module ?audit_fields ?default_mount ?default_path_policy ?enable_sentinel_parsing ?enabled ?id
  ?label_to_path_policy ?namespace ?(authenticators = []) ~backend __id =
  let (r : _ Tf_core.resource) =
    make ?audit_fields ?default_mount ?default_path_policy ?enable_sentinel_parsing ?enabled ?id ?label_to_path_policy
      ?namespace ~authenticators ~backend __id
  in
