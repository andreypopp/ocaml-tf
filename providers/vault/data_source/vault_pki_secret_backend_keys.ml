...
let _ = fun (_ : vault_pki_secret_backend_keys) -> ()

let yojson_of_vault_pki_secret_backend_keys =
  (function
   | { backend = v_backend; id = v_id; namespace = v_namespace } ->
     ...
     in
     `Assoc bnds
    : vault_pki_secret_backend_keys -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_vault_pki_secret_backend_keys

[@@@deriving.end]

let vault_pki_secret_backend_keys ?id ?namespace ~backend () =
  ({ backend; id; namespace } : vault_pki_secret_backend_keys)
...
let make ?id ?namespace ~backend __id =
  let __type = "vault_pki_secret_backend_keys" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    Tf_core.id = __id;
    type_ = __type;
    json = yojson_of_vault_pki_secret_backend_keys (vault_pki_secret_backend_keys ?id ?namespace ~backend ());
    attrs = __attrs;
  }
