...
let _ = fun (_ : vault_identity_mfa_pingid) -> ()

let yojson_of_vault_identity_mfa_pingid =
  (function
   | {
       id = v_id;
     ...
     in
     `Assoc bnds
    : vault_identity_mfa_pingid -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_vault_identity_mfa_pingid

[@@@deriving.end]

let vault_identity_mfa_pingid ?id ?namespace ?username_format ~settings_file_base64 () =
  ({ id; namespace; settings_file_base64; username_format } : vault_identity_mfa_pingid)
...
let make ?id ?namespace ?username_format ~settings_file_base64 __id =
  let __type = "vault_identity_mfa_pingid" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_vault_identity_mfa_pingid
        (vault_identity_mfa_pingid ?id ?namespace ?username_format ~settings_file_base64 ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?namespace ?username_format ~settings_file_base64 __id =
  let (r : _ Tf_core.resource) = make ?id ?namespace ?username_format ~settings_file_base64 __id in
