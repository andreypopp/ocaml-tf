(* DO NOT EDIT, GENERATED AUTOMATICALLY *)

open! Tf_core

type vault_secrets_sync_gcp_destination = {
  credentials : string prop option; [@option]
  custom_tags : string prop Tf_core.assoc option; [@option]
  granularity : string prop option; [@option]
  id : string prop option; [@option]
  name : string prop;
  namespace : string prop option; [@option]
  project_id : string prop option; [@option]
  secret_name_template : string prop option; [@option]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : vault_secrets_sync_gcp_destination) -> ()

let yojson_of_vault_secrets_sync_gcp_destination =
  (function
   | {
       credentials = v_credentials;
       custom_tags = v_custom_tags;
       granularity = v_granularity;
       id = v_id;
       name = v_name;
       namespace = v_namespace;
       project_id = v_project_id;
       secret_name_template = v_secret_name_template;
     } ->
     let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list = [] in
     let bnds =
       match v_secret_name_template with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_string v in
         let bnd = "secret_name_template", arg in
         bnd :: bnds
     in
     let bnds =
       match v_project_id with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_string v in
         let bnd = "project_id", arg in
         bnd :: bnds
     in
     let bnds =
       match v_namespace with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_string v in
         let bnd = "namespace", arg in
         bnd :: bnds
     in
     let bnds =
       let arg = yojson_of_prop yojson_of_string v_name in
       ("name", arg) :: bnds
     in
     let bnds =
       match v_id with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_string v in
         let bnd = "id", arg in
         bnd :: bnds
     in
     let bnds =
       match v_granularity with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_string v in
         let bnd = "granularity", arg in
         bnd :: bnds
     in
     let bnds =
       match v_custom_tags with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = Tf_core.yojson_of_assoc (yojson_of_prop yojson_of_string) v in
         let bnd = "custom_tags", arg in
         bnd :: bnds
     in
     let bnds =
       match v_credentials with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_string v in
         let bnd = "credentials", arg in
         bnd :: bnds
     in
     `Assoc bnds
    : vault_secrets_sync_gcp_destination -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_vault_secrets_sync_gcp_destination

[@@@deriving.end]

let vault_secrets_sync_gcp_destination ?credentials ?custom_tags ?granularity ?id ?namespace ?project_id
  ?secret_name_template ~name () =
  ({ credentials; custom_tags; granularity; id; name; namespace; project_id; secret_name_template }
    : vault_secrets_sync_gcp_destination)

type t = {
  tf_name : string;
  credentials : string prop;
  custom_tags : string Tf_core.assoc prop;
  granularity : string prop;
  id : string prop;
  name : string prop;
  namespace : string prop;
  project_id : string prop;
  secret_name_template : string prop;
  type_ : string prop;
}

let make ?credentials ?custom_tags ?granularity ?id ?namespace ?project_id ?secret_name_template ~name __id =
  let __type = "vault_secrets_sync_gcp_destination" in
  let __attrs =
    ({
       tf_name = __id;
       credentials = Prop.computed __type __id "credentials";
       custom_tags = Prop.computed __type __id "custom_tags";
       granularity = Prop.computed __type __id "granularity";
       id = Prop.computed __type __id "id";
       name = Prop.computed __type __id "name";
       namespace = Prop.computed __type __id "namespace";
       project_id = Prop.computed __type __id "project_id";
       secret_name_template = Prop.computed __type __id "secret_name_template";
       type_ = Prop.computed __type __id "type";
     }
      : t)
  in
  {
    Tf_core.id = __id;
    type_ = __type;
    json =
      yojson_of_vault_secrets_sync_gcp_destination
        (vault_secrets_sync_gcp_destination ?credentials ?custom_tags ?granularity ?id ?namespace ?project_id
           ?secret_name_template ~name ());
    attrs = __attrs;
  }

let register ?tf_module ?credentials ?custom_tags ?granularity ?id ?namespace ?project_id ?secret_name_template ~name
  __id =
  let (r : _ Tf_core.resource) =
    make ?credentials ?custom_tags ?granularity ?id ?namespace ?project_id ?secret_name_template ~name __id
  in
  Resource.add ?tf_module ~type_:r.type_ ~id:r.id r.json;
  r.attrs
