...
type vault_pki_secret_backend_intermediate_cert_request = {
  add_basic_constraints : bool prop option; [@option]
...
let _ = fun (_ : vault_pki_secret_backend_intermediate_cert_request) -> ()

let yojson_of_vault_pki_secret_backend_intermediate_cert_request =
  (function
   | {
       add_basic_constraints = v_add_basic_constraints;
     ...
     in
     let bnds =
       match v_add_basic_constraints with
       | Ppx_yojson_conv_lib.Option.None -> bnds
       | Ppx_yojson_conv_lib.Option.Some v ->
         let arg = yojson_of_prop yojson_of_bool v in
         let bnd = "add_basic_constraints", arg in
         bnd :: bnds
     in
     `Assoc bnds
    : vault_pki_secret_backend_intermediate_cert_request -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_vault_pki_secret_backend_intermediate_cert_request

[@@@deriving.end]

let vault_pki_secret_backend_intermediate_cert_request ?add_basic_constraints ?alt_names ?country ?exclude_cn_from_sans
  ?format ?id ?ip_sans ?key_bits ?key_name ?key_ref ?key_type ?key_usage ?locality ?managed_key_id ?managed_key_name
  ?namespace ?organization ?other_sans ?ou ?postal_code ?private_key_format ?province ?serial_number ?signature_bits
  ?street_address ?uri_sans ~backend ~common_name ~type_ () =
  ({
...
let make ?add_basic_constraints ?alt_names ?country ?exclude_cn_from_sans ?format ?id ?ip_sans ?key_bits ?key_name
  ?key_ref ?key_type ?key_usage ?locality ?managed_key_id ?managed_key_name ?namespace ?organization ?other_sans ?ou
  ?postal_code ?private_key_format ?province ?serial_number ?signature_bits ?street_address ?uri_sans ~backend
  ~common_name ~type_ __id =
  let __type = "vault_pki_secret_backend_intermediate_cert_request" in
  let __attrs =
    ({
       tf_name = __id;
       add_basic_constraints = Prop.computed __type __id "add_basic_constraints";
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_vault_pki_secret_backend_intermediate_cert_request
        (vault_pki_secret_backend_intermediate_cert_request ?add_basic_constraints ?alt_names ?country
           ?exclude_cn_from_sans ?format ?id ?ip_sans ?key_bits ?key_name ?key_ref ?key_type ?key_usage ?locality
           ?managed_key_id ?managed_key_name ?namespace ?organization ?other_sans ?ou ?postal_code ?private_key_format
           ?province ?serial_number ?signature_bits ?street_address ?uri_sans ~backend ~common_name ~type_ ());
    attrs = __attrs;
  }

let register ?tf_module ?add_basic_constraints ?alt_names ?country ?exclude_cn_from_sans ?format ?id ?ip_sans ?key_bits
  ?key_name ?key_ref ?key_type ?key_usage ?locality ?managed_key_id ?managed_key_name ?namespace ?organization
  ?other_sans ?ou ?postal_code ?private_key_format ?province ?serial_number ?signature_bits ?street_address ?uri_sans
  ~backend ~common_name ~type_ __id =
  let (r : _ Tf_core.resource) =
    make ?add_basic_constraints ?alt_names ?country ?exclude_cn_from_sans ?format ?id ?ip_sans ?key_bits ?key_name
      ?key_ref ?key_type ?key_usage ?locality ?managed_key_id ?managed_key_name ?namespace ?organization ?other_sans ?ou
      ?postal_code ?private_key_format ?province ?serial_number ?signature_bits ?street_address ?uri_sans ~backend
      ~common_name ~type_ __id
  in
