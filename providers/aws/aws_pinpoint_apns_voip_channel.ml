...
let _ = fun (_ : aws_pinpoint_apns_voip_channel) -> ()

let yojson_of_aws_pinpoint_apns_voip_channel =
  (function
   | {
...
let _ = yojson_of_aws_pinpoint_apns_voip_channel
...
let make ?bundle_id ?certificate ?default_authentication_method
    ...
    ~application_id __id =
  let __type = "aws_pinpoint_apns_voip_channel" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_pinpoint_apns_voip_channel
