...
let _ = fun (_ : aws_vpc_ipam_scope) -> ()

let yojson_of_aws_vpc_ipam_scope =
  (function
   | {
       ...
       in
       `Assoc bnds
    : aws_vpc_ipam_scope -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_vpc_ipam_scope
...
let make ?description ?id ?tags ?tags_all ?timeouts ~ipam_id __id =
  ...
  let __attrs =
    ({
       ...
       pool_count = Prop.computed __type __id "pool_count";
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_vpc_ipam_scope
        (aws_vpc_ipam_scope ?description ?id ?tags ?tags_all
           ?timeouts ~ipam_id ());
    attrs = __attrs;
  }
...
let register ?tf_module ?description ?id ?tags ?tags_all ?timeouts
  ...
  let (r : _ Tf_core.resource) =
    make ?description ?id ?tags ?tags_all ?timeouts ~ipam_id __id
  in
