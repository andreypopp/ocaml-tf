...
let _ = fun (_ : aws_opsworks_memcached_layer) -> ()

let yojson_of_aws_opsworks_memcached_layer =
  (function
   | {
       allocated_memory = v_allocated_memory;
       ...
       in
       let bnds =
         match v_allocated_memory with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_float v in
             let bnd = "allocated_memory", arg in
             bnd :: bnds
       in
       `Assoc bnds
...
let _ = yojson_of_aws_opsworks_memcached_layer
...
let aws_opsworks_memcached_layer ?allocated_memory
    ...
    ~load_based_auto_scaling () : aws_opsworks_memcached_layer =
  {
    allocated_memory;
...
let make ?allocated_memory ?auto_assign_elastic_ips
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_opsworks_memcached_layer
        (aws_opsworks_memcached_layer ?allocated_memory
           ...
           ?instance_shutdown_timeout ?name ?system_packages ?tags
...
let register ?tf_module ?allocated_memory ?auto_assign_elastic_ips
    ?auto_assign_public_ips ?auto_healing ?custom_configure_recipes
    ?custom_deploy_recipes ?custom_instance_profile_arn ?custom_json
    ?custom_security_group_ids ?custom_setup_recipes
    ?custom_shutdown_recipes ?custom_undeploy_recipes
  ...
  let (r : _ Tf_core.resource) =
    make ?allocated_memory ?auto_assign_elastic_ips
      ?auto_assign_public_ips ?auto_healing ?custom_configure_recipes
      ?custom_deploy_recipes ?custom_instance_profile_arn
