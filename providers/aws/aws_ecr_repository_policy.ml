...
let _ = fun (_ : aws_ecr_repository_policy) -> ()

let yojson_of_aws_ecr_repository_policy =
  (function
   | { id = v_id; policy = v_policy; repository = v_repository } ->
       ...
       in
       `Assoc bnds
    : aws_ecr_repository_policy -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_ecr_repository_policy

[@@@deriving.end]

let aws_ecr_repository_policy ?id ~policy ~repository () :
...
let make ?id ~policy ~repository __id =
  let __type = "aws_ecr_repository_policy" in
  let __attrs =
    ({
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_ecr_repository_policy
        (aws_ecr_repository_policy ?id ~policy ~repository ());
    attrs = __attrs;
  }
