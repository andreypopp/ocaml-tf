(* DO NOT EDIT, GENERATED AUTOMATICALLY *)

open! Tf_core

type aws_iam_role_policy_attachment = {
  id : string prop option; [@option]
  policy_arn : string prop;
  role : string prop;
}
[@@deriving_inline yojson_of]

let _ = fun (_ : aws_iam_role_policy_attachment) -> ()

let yojson_of_aws_iam_role_policy_attachment =
  (function
   | { id = v_id; policy_arn = v_policy_arn; role = v_role } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_role in
         ("role", arg) :: bnds
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_policy_arn in
         ("policy_arn", arg) :: bnds
       in
       let bnds =
         match v_id with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "id", arg in
             bnd :: bnds
       in
       `Assoc bnds
    : aws_iam_role_policy_attachment ->
      Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_iam_role_policy_attachment

[@@@deriving.end]

let aws_iam_role_policy_attachment ?id ~policy_arn ~role () :
    aws_iam_role_policy_attachment =
  { id; policy_arn; role }

type t = {
  tf_name : string;
  id : string prop;
  policy_arn : string prop;
  role : string prop;
}

let make ?id ~policy_arn ~role __id =
  let __type = "aws_iam_role_policy_attachment" in
  let __attrs =
    ({
       tf_name = __id;
       id = Prop.computed __type __id "id";
       policy_arn = Prop.computed __type __id "policy_arn";
       role = Prop.computed __type __id "role";
     }
      : t)
  in
  {
    Tf_core.id = __id;
    type_ = __type;
    json =
      yojson_of_aws_iam_role_policy_attachment
        (aws_iam_role_policy_attachment ?id ~policy_arn ~role ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~policy_arn ~role __id =
  let (r : _ Tf_core.resource) = make ?id ~policy_arn ~role __id in
  Resource.add ?tf_module ~type_:r.type_ ~id:r.id r.json;
  r.attrs
