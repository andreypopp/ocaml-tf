...
let aws_docdb_cluster_parameter_group ?description ?id ?name
...
let make ?description ?id ?name ?name_prefix ?tags ?tags_all ~family
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_docdb_cluster_parameter_group
        (aws_docdb_cluster_parameter_group ?description ?id ?name
           ?name_prefix ?tags ?tags_all ~family ~parameter ());
    attrs = __attrs;
  }
