...
let _ = fun (_ : aws_lb_listener_rule) -> ()

let yojson_of_aws_lb_listener_rule =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : aws_lb_listener_rule -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_lb_listener_rule
...
let make ?id ?priority ?tags ?tags_all ~listener_arn ~action
    ~condition __id =
  let __type = "aws_lb_listener_rule" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_lb_listener_rule
        (aws_lb_listener_rule ?id ?priority ?tags ?tags_all
