...
let _ = fun (_ : aws_keyspaces_keyspace) -> ()

let yojson_of_aws_keyspaces_keyspace =
  (function
   | {
       id = v_id;
       name = v_name;
       ...
       in
       `Assoc bnds
    : aws_keyspaces_keyspace -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_keyspaces_keyspace
