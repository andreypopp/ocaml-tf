...
let register ?tf_module ?aws_account_id ?id ?namespace ~group_name ~member_name __resource_id =
  let __resource_type = "aws_quicksight_group_membership" in
  let __resource = aws_quicksight_group_membership ?aws_account_id ?id ?namespace ~group_name ~member_name () in
