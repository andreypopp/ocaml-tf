...
let _ = fun (_ : aws_opensearch_domain_policy) -> ()

let yojson_of_aws_opensearch_domain_policy =
  (function
   | {
...
let _ = yojson_of_aws_opensearch_domain_policy
...
let make ?id ?timeouts ~access_policies ~domain_name __id =
  let __type = "aws_opensearch_domain_policy" in
  let __attrs =
    ({
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_opensearch_domain_policy
