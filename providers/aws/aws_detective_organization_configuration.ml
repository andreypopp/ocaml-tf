(* DO NOT EDIT, GENERATED AUTOMATICALLY *)

open! Tf_core

type aws_detective_organization_configuration = {
  auto_enable : bool prop;
  graph_arn : string prop;
  id : string prop option; [@option]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : aws_detective_organization_configuration) -> ()

let yojson_of_aws_detective_organization_configuration =
  (function
   | {
       auto_enable = v_auto_enable;
       graph_arn = v_graph_arn;
       id = v_id;
     } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         match v_id with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "id", arg in
             bnd :: bnds
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_graph_arn in
         ("graph_arn", arg) :: bnds
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_bool v_auto_enable in
         ("auto_enable", arg) :: bnds
       in
       `Assoc bnds
    : aws_detective_organization_configuration ->
      Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_detective_organization_configuration

[@@@deriving.end]

let aws_detective_organization_configuration ?id ~auto_enable
    ~graph_arn () : aws_detective_organization_configuration =
  { auto_enable; graph_arn; id }

type t = {
  tf_name : string;
  auto_enable : bool prop;
  graph_arn : string prop;
  id : string prop;
}

let make ?id ~auto_enable ~graph_arn __id =
  let __type = "aws_detective_organization_configuration" in
  let __attrs =
    ({
       tf_name = __id;
       auto_enable = Prop.computed __type __id "auto_enable";
       graph_arn = Prop.computed __type __id "graph_arn";
       id = Prop.computed __type __id "id";
     }
      : t)
  in
  {
    Tf_core.id = __id;
    type_ = __type;
    json =
      yojson_of_aws_detective_organization_configuration
        (aws_detective_organization_configuration ?id ~auto_enable
           ~graph_arn ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~auto_enable ~graph_arn __id =
  let (r : _ Tf_core.resource) =
    make ?id ~auto_enable ~graph_arn __id
  in
  Resource.add ?tf_module ~type_:r.type_ ~id:r.id r.json;
  r.attrs
