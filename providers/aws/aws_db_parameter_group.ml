...
let aws_db_parameter_group ?description ?id ?name ?name_prefix ?tags
    ?tags_all ~family ~parameter () : aws_db_parameter_group =
  {
    description;
    family;
    id;
...
let register ?tf_module ?description ?id ?name ?name_prefix ?tags
  ...
  let __resource_type = "aws_db_parameter_group" in
  let __resource =
    aws_db_parameter_group ?description ?id ?name ?name_prefix ?tags
      ?tags_all ~family ~parameter ()
  in
  Resource.add ?tf_module ~type_:__resource_type ~id:__resource_id
    (yojson_of_aws_db_parameter_group __resource);
