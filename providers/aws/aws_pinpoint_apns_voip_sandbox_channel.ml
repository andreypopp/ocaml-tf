...
let make ?bundle_id ?certificate ?default_authentication_method
    ...
    ~application_id __id =
  let __type = "aws_pinpoint_apns_voip_sandbox_channel" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_pinpoint_apns_voip_sandbox_channel
        (aws_pinpoint_apns_voip_sandbox_channel ?bundle_id
           ?certificate ?default_authentication_method ?enabled ?id
           ?private_key ?team_id ?token_key ?token_key_id
           ~application_id ());
    attrs = __attrs;
  }
