...
let _ = fun (_ : aws_dx_hosted_public_virtual_interface) -> ()

let yojson_of_aws_dx_hosted_public_virtual_interface =
  (function
   | {
       ...
       route_filter_prefixes = v_route_filter_prefixes;
       vlan = v_vlan;
       timeouts = v_timeouts;
     } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_route_filter_prefixes then bnds
         else
           let arg =
...
let _ = yojson_of_aws_dx_hosted_public_virtual_interface
