...
let _ = fun (_ : aws_dx_hosted_private_virtual_interface) -> ()

let yojson_of_aws_dx_hosted_private_virtual_interface =
  (function
   | {
...
let _ = yojson_of_aws_dx_hosted_private_virtual_interface
...
let make ?amazon_address ?bgp_auth_key ?customer_address ?id ?mtu
  ...
  let __type = "aws_dx_hosted_private_virtual_interface" in
  let __attrs =
    ({
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_dx_hosted_private_virtual_interface
