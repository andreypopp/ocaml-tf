...
let _ = fun (_ : aws_docdb_cluster_snapshot) -> ()

let yojson_of_aws_docdb_cluster_snapshot =
  (function
   | {
       ...
       in
       `Assoc bnds
    : aws_docdb_cluster_snapshot -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_docdb_cluster_snapshot
...
let make ?id ?timeouts ~db_cluster_identifier
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_docdb_cluster_snapshot
        (aws_docdb_cluster_snapshot ?id ?timeouts
