...
let _ = fun (_ : aws_opsworks_custom_layer) -> ()

let yojson_of_aws_opsworks_custom_layer =
  (function
   | {
       ...
       in
       `Assoc bnds
    : aws_opsworks_custom_layer -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_opsworks_custom_layer
...
let aws_opsworks_custom_layer ?auto_assign_elastic_ips
    ...
    ~name ~short_name ~stack_id ~ebs_volume () :
...
let make ?auto_assign_elastic_ips ?auto_assign_public_ips
    ...
    ?system_packages ?tags ?tags_all ?use_ebs_optimized_instances
    ?(cloudwatch_configuration = []) ?(load_based_auto_scaling = [])
    ~name ~short_name ~stack_id ~ebs_volume __id =
  let __type = "aws_opsworks_custom_layer" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_opsworks_custom_layer
        (aws_opsworks_custom_layer ?auto_assign_elastic_ips
           ...
           ~load_based_auto_scaling ~name ~short_name ~stack_id
           ~ebs_volume ());
    attrs = __attrs;
  }
...
let register ?tf_module ?auto_assign_elastic_ips
  ...
  let (r : _ Tf_core.resource) =
    make ?auto_assign_elastic_ips ?auto_assign_public_ips
      ...
      ?install_updates_on_boot ?instance_shutdown_timeout
      ?system_packages ?tags ?tags_all ?use_ebs_optimized_instances
      ~cloudwatch_configuration ~load_based_auto_scaling ~name
      ~short_name ~stack_id ~ebs_volume __id
  in
