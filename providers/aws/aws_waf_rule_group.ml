...
let _ = fun (_ : aws_waf_rule_group) -> ()

let yojson_of_aws_waf_rule_group =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : aws_waf_rule_group -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_waf_rule_group
...
let aws_waf_rule_group ?id ?tags ?tags_all ~metric_name ~name
    ~activated_rule () : aws_waf_rule_group =
...
let make ?id ?tags ?tags_all ~metric_name ~name ~activated_rule __id
    =
  let __type = "aws_waf_rule_group" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_waf_rule_group
        (aws_waf_rule_group ?id ?tags ?tags_all ~metric_name ~name
           ~activated_rule ());
    attrs = __attrs;
  }
