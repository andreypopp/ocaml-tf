...
let _ = fun (_ : aws_route53_vpc_association_authorization) -> ()

let yojson_of_aws_route53_vpc_association_authorization =
  (function
   | {
       id = v_id;
       vpc_id = v_vpc_id;
...
let _ = yojson_of_aws_route53_vpc_association_authorization
...
let make ?id ?vpc_region ~vpc_id ~zone_id __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_route53_vpc_association_authorization
        (aws_route53_vpc_association_authorization ?id ?vpc_region
           ~vpc_id ~zone_id ());
    attrs = __attrs;
  }
