...
let _ = fun (_ : aws_ses_receipt_rule_set) -> ()

let yojson_of_aws_ses_receipt_rule_set =
  (function
   | { id = v_id; rule_set_name = v_rule_set_name } ->
       ...
       in
       `Assoc bnds
    : aws_ses_receipt_rule_set -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_ses_receipt_rule_set
...
let make ?id ~rule_set_name __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_ses_receipt_rule_set
        (aws_ses_receipt_rule_set ?id ~rule_set_name ());
    attrs = __attrs;
  }
