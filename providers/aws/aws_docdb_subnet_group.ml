...
let _ = fun (_ : aws_docdb_subnet_group) -> ()

let yojson_of_aws_docdb_subnet_group =
  (function
   | {
       ...
       in
       `Assoc bnds
    : aws_docdb_subnet_group -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_docdb_subnet_group
...
let make ?description ?id ?name ?name_prefix ?tags ?tags_all
    ~subnet_ids __id =
  let __type = "aws_docdb_subnet_group" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_docdb_subnet_group
        (aws_docdb_subnet_group ?description ?id ?name ?name_prefix
