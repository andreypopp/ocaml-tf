...
let _ = fun (_ : table_with_columns__column_wildcard) -> ()

let yojson_of_table_with_columns__column_wildcard =
  (function
...
let _ = yojson_of_table_with_columns__column_wildcard
   ...
   | {
       ...
       let bnds =
         if Stdlib.( = ) [] v_column_wildcard then bnds
           ...
           let arg =
             (yojson_of_list
                yojson_of_table_with_columns__column_wildcard)
               v_column_wildcard
           in
...
let _ = fun (_ : aws_lakeformation_resource_lf_tag) -> ()

let yojson_of_aws_lakeformation_resource_lf_tag =
  (function
   | {
       catalog_id = v_catalog_id;
       database = v_database;
...
let _ = yojson_of_aws_lakeformation_resource_lf_tag
...
let table_with_columns__column_wildcard ?excluded_column_names () :
    table_with_columns__column_wildcard =
  { excluded_column_names }

let table_with_columns ?catalog_id ?column_names
    ?(column_wildcard = []) ~database_name ~name () :
    table_with_columns =
  { catalog_id; column_names; database_name; name; column_wildcard }

let timeouts ?create ?delete () : timeouts = { create; delete }

let aws_lakeformation_resource_lf_tag ?catalog_id ?(database = [])
    ?(lf_tag = []) ?(table = []) ?(table_with_columns = []) ?timeouts
    () : aws_lakeformation_resource_lf_tag =
  {
    catalog_id;
    database;
...
let make ?catalog_id ?(database = []) ?(lf_tag = []) ?(table = [])
    ?(table_with_columns = []) ?timeouts __id =
  let __type = "aws_lakeformation_resource_lf_tag" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_lakeformation_resource_lf_tag
        (aws_lakeformation_resource_lf_tag ?catalog_id ~database
           ~lf_tag ~table ~table_with_columns ?timeouts ());
    attrs = __attrs;
  }

let register ?tf_module ?catalog_id ?(database = []) ?(lf_tag = [])
    ?(table = []) ?(table_with_columns = []) ?timeouts __id =
  let (r : _ Tf_core.resource) =
    make ?catalog_id ~database ~lf_tag ~table ~table_with_columns
      ?timeouts __id
  in
