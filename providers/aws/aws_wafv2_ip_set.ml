...
let register ?tf_module ?addresses ?description ?id ?tags ?tags_all ~ip_address_version ~name ~scope __resource_id =
  let __resource_type = "aws_wafv2_ip_set" in
  let __resource = aws_wafv2_ip_set ?addresses ?description ?id ?tags ?tags_all ~ip_address_version ~name ~scope () in
  Resource.add ?tf_module ~type_:__resource_type ~id:__resource_id
    (yojson_of_aws_wafv2_ip_set __resource);
  let __resource_attributes = ({
    addresses = Prop.computed __resource_type __resource_id "addresses";
    arn = Prop.computed __resource_type __resource_id "arn";
    description = Prop.computed __resource_type __resource_id "description";
    id = Prop.computed __resource_type __resource_id "id";
    ip_address_version = Prop.computed __resource_type __resource_id "ip_address_version";
