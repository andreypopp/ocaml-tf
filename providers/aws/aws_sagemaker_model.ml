...
let yojson_of_container__image_config =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_repository_auth_config then bnds
         else
           let arg =
             (yojson_of_list
...
let yojson_of_container__model_data_source =
  (function
   | { s3_data_source = v_s3_data_source } ->
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_s3_data_source then bnds
         else
           let arg =
             (yojson_of_list
...
let yojson_of_container =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_model_data_source then bnds
         else
           let arg =
...
let yojson_of_aws_sagemaker_model =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_primary_container then bnds
         else
           let arg =
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_inference_execution_config then bnds
         else
           let arg =
