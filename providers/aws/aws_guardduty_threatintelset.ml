...
let _ = fun (_ : aws_guardduty_threatintelset) -> ()

let yojson_of_aws_guardduty_threatintelset =
  (function
   | {
       activate = v_activate;
...
let _ = yojson_of_aws_guardduty_threatintelset
...
let make ?id ?tags ?tags_all ~activate ~detector_id ~format ~location
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_guardduty_threatintelset
