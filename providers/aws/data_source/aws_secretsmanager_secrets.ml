...
let _ = fun (_ : aws_secretsmanager_secrets) -> ()

let yojson_of_aws_secretsmanager_secrets =
  (function
   | { id = v_id; filter = v_filter } ->
       ...
       in
       `Assoc bnds
    : aws_secretsmanager_secrets -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_secretsmanager_secrets

[@@@deriving.end]

let filter ~name ~values () : filter = { name; values }

let aws_secretsmanager_secrets ?id ~filter () :
...
let make ?id ~filter __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_secretsmanager_secrets
        (aws_secretsmanager_secrets ?id ~filter ());
    attrs = __attrs;
  }
