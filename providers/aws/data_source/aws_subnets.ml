...
let _ = fun (_ : aws_subnets) -> ()

let yojson_of_aws_subnets =
  (function
   | {
       id = v_id;
       tags = v_tags;
       ...
       in
       `Assoc bnds
    : aws_subnets -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_subnets
...
let aws_subnets ?id ?tags ?timeouts ~filter () : aws_subnets =
...
let make ?id ?tags ?timeouts ~filter __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_subnets
        (aws_subnets ?id ?tags ?timeouts ~filter ());
    attrs = __attrs;
  }
