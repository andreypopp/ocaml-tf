(* DO NOT EDIT, GENERATED AUTOMATICALLY *)

open! Tf_core

type aws_iot_registration_code = {
  id : string prop option; [@option]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : aws_iot_registration_code) -> ()

let yojson_of_aws_iot_registration_code =
  (function
   | { id = v_id } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         match v_id with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "id", arg in
             bnd :: bnds
       in
       `Assoc bnds
    : aws_iot_registration_code -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_iot_registration_code

[@@@deriving.end]

let aws_iot_registration_code ?id () : aws_iot_registration_code =
  { id }

type t = {
  tf_name : string;
  id : string prop;
  registration_code : string prop;
}

let make ?id __id =
  let __type = "aws_iot_registration_code" in
  let __attrs =
    ({
       tf_name = __id;
       id = Prop.computed __type __id "id";
       registration_code =
         Prop.computed __type __id "registration_code";
     }
      : t)
  in
  {
    Tf_core.id = __id;
    type_ = __type;
    json =
      yojson_of_aws_iot_registration_code
        (aws_iot_registration_code ?id ());
    attrs = __attrs;
  }

let register ?tf_module ?id __id =
  let (r : _ Tf_core.resource) = make ?id __id in
  Data.add ?tf_module ~type_:r.type_ ~id:r.id r.json;
  r.attrs
