...
let _ = fun (_ : aws_iam_roles) -> ()

let yojson_of_aws_iam_roles =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : aws_iam_roles -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_iam_roles

[@@@deriving.end]

let aws_iam_roles ?id ?name_regex ?path_prefix () : aws_iam_roles =
...
let make ?id ?name_regex ?path_prefix __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_iam_roles
        (aws_iam_roles ?id ?name_regex ?path_prefix ());
    attrs = __attrs;
  }
