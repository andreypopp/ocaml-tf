...
let _ = fun (_ : aws_nat_gateways) -> ()

let yojson_of_aws_nat_gateways =
  (function
   | {
       id = v_id;
       tags = v_tags;
       ...
       in
       `Assoc bnds
    : aws_nat_gateways -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_nat_gateways
...
let aws_nat_gateways ?id ?tags ?vpc_id ?timeouts ~filter () :
    aws_nat_gateways =
...
let make ?id ?tags ?vpc_id ?timeouts ~filter __id =
  let __type = "aws_nat_gateways" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_nat_gateways
        (aws_nat_gateways ?id ?tags ?vpc_id ?timeouts ~filter ());
    attrs = __attrs;
  }
