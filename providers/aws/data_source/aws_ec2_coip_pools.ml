...
let _ = fun (_ : aws_ec2_coip_pools) -> ()

let yojson_of_aws_ec2_coip_pools =
  (function
   | {
       id = v_id;
       tags = v_tags;
       ...
       in
       `Assoc bnds
    : aws_ec2_coip_pools -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_ec2_coip_pools
...
let aws_ec2_coip_pools ?id ?tags ?timeouts ~filter () :
    aws_ec2_coip_pools =
...
let make ?id ?tags ?timeouts ~filter __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_ec2_coip_pools
        (aws_ec2_coip_pools ?id ?tags ?timeouts ~filter ());
    attrs = __attrs;
  }
