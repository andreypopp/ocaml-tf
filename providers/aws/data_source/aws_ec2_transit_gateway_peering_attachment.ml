...
let aws_ec2_transit_gateway_peering_attachment ?id ?tags ?timeouts
    ~filter () : aws_ec2_transit_gateway_peering_attachment =
  ...
  in
  {
    ...
    json =
      yojson_of_aws_ec2_transit_gateway_peering_attachment
        (aws_ec2_transit_gateway_peering_attachment ?id ?tags
           ?timeouts ~filter ());
    attrs = __attrs;
  }
