...
let _ = fun (_ : aws_vpcs) -> ()

let yojson_of_aws_vpcs =
  (function
   | {
       id = v_id;
       tags = v_tags;
       ...
       in
       `Assoc bnds
    : aws_vpcs -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_vpcs
...
let aws_vpcs ?id ?tags ?timeouts ~filter () : aws_vpcs =
...
let make ?id ?tags ?timeouts ~filter __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_vpcs (aws_vpcs ?id ?tags ?timeouts ~filter ());
    attrs = __attrs;
  }
