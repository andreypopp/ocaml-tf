...
let _ = fun (_ : aws_fsx_ontap_storage_virtual_machines) -> ()

let yojson_of_aws_fsx_ontap_storage_virtual_machines =
  (function
...
let _ = yojson_of_aws_fsx_ontap_storage_virtual_machines

[@@@deriving.end]

let filter ~name ~values () : filter = { name; values }

let aws_fsx_ontap_storage_virtual_machines ?id ~filter () :
    aws_fsx_ontap_storage_virtual_machines =
  { id; filter }
...
let make ?id ~filter __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_fsx_ontap_storage_virtual_machines
        (aws_fsx_ontap_storage_virtual_machines ?id ~filter ());
    attrs = __attrs;
  }
