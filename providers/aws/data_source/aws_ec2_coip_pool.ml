...
let _ = fun (_ : aws_ec2_coip_pool) -> ()

let yojson_of_aws_ec2_coip_pool =
  (function
   | {
       id = v_id;
       ...
       in
       `Assoc bnds
    : aws_ec2_coip_pool -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_ec2_coip_pool
...
let aws_ec2_coip_pool ?id ?local_gateway_route_table_id ?pool_id
    ?tags ?timeouts ~filter () : aws_ec2_coip_pool =
  {
    id;
...
let make ?id ?local_gateway_route_table_id ?pool_id ?tags ?timeouts
  ...
  let __attrs =
    ({
       ...
       pool_cidrs = Prop.computed __type __id "pool_cidrs";
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_ec2_coip_pool
        (aws_ec2_coip_pool ?id ?local_gateway_route_table_id ?pool_id
           ?tags ?timeouts ~filter ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?local_gateway_route_table_id ?pool_id
    ?tags ?timeouts ~filter __id =
  let (r : _ Tf_core.resource) =
    make ?id ?local_gateway_route_table_id ?pool_id ?tags ?timeouts
      ~filter __id
  in
