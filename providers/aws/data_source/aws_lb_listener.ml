...
let _ = fun (_ : aws_lb_listener) -> ()

let yojson_of_aws_lb_listener =
  (function
   | {
       arn = v_arn;
       id = v_id;
       ...
       in
       `Assoc bnds
    : aws_lb_listener -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_lb_listener

[@@@deriving.end]

let timeouts ?read () : timeouts = { read }

let aws_lb_listener ?arn ?id ?load_balancer_arn ?port ?tags ?timeouts
    () : aws_lb_listener =
  ...
  in
  {
    ...
    json =
      yojson_of_aws_lb_listener
        (aws_lb_listener ?arn ?id ?load_balancer_arn ?port ?tags
           ?timeouts ());
    attrs = __attrs;
  }
