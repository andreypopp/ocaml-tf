...
let _ = fun (_ : aws_ssm_maintenance_windows) -> ()

let yojson_of_aws_ssm_maintenance_windows =
  (function
...
let _ = yojson_of_aws_ssm_maintenance_windows

[@@@deriving.end]

let filter ~name ~values () : filter = { name; values }

let aws_ssm_maintenance_windows ?id ~filter () :
    aws_ssm_maintenance_windows =
  { id; filter }
...
let make ?id ~filter __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_ssm_maintenance_windows
        (aws_ssm_maintenance_windows ?id ~filter ());
    attrs = __attrs;
  }
