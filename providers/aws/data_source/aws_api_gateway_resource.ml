...
let yojson_of_aws_api_gateway_resource =
  (function
   | { id = v_id; path = v_path; rest_api_id = v_rest_api_id } ->
...
let aws_api_gateway_resource ?id ~path ~rest_api_id () :
...
let make ?id ~path ~rest_api_id __id =
  ...
  in
  {
    ...
    json =
      yojson_of_aws_api_gateway_resource
        (aws_api_gateway_resource ?id ~path ~rest_api_id ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~path ~rest_api_id __id =
  let (r : _ Tf_core.resource) = make ?id ~path ~rest_api_id __id in
