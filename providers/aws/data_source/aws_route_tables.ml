...
let _ = fun (_ : aws_route_tables) -> ()

let yojson_of_aws_route_tables =
  (function
   | {
       id = v_id;
       tags = v_tags;
       ...
       in
       `Assoc bnds
    : aws_route_tables -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_route_tables
...
let aws_route_tables ?id ?tags ?vpc_id ?timeouts ~filter () :
    aws_route_tables =
...
let make ?id ?tags ?vpc_id ?timeouts ~filter __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_route_tables
        (aws_route_tables ?id ?tags ?vpc_id ?timeouts ~filter ());
    attrs = __attrs;
  }
