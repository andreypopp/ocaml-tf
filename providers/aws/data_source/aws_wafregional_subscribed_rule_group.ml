...
let _ = fun (_ : aws_wafregional_subscribed_rule_group) -> ()

let yojson_of_aws_wafregional_subscribed_rule_group =
  (function
...
let _ = yojson_of_aws_wafregional_subscribed_rule_group

[@@@deriving.end]

let aws_wafregional_subscribed_rule_group ?id ?metric_name ?name () :
    aws_wafregional_subscribed_rule_group =
  { id; metric_name; name }
...
let make ?id ?metric_name ?name __id =
  let __type = "aws_wafregional_subscribed_rule_group" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_wafregional_subscribed_rule_group
        (aws_wafregional_subscribed_rule_group ?id ?metric_name ?name
           ());
    attrs = __attrs;
  }
