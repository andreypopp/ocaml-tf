...
let _ = fun (_ : aws_networkmanager_sites) -> ()

let yojson_of_aws_networkmanager_sites =
  (function
   | {
       ...
       in
       `Assoc bnds
    : aws_networkmanager_sites -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_networkmanager_sites

[@@@deriving.end]

let aws_networkmanager_sites ?id ?tags ~global_network_id () :
...
let make ?id ?tags ~global_network_id __id =
  let __type = "aws_networkmanager_sites" in
  let __attrs =
    ({
       global_network_id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_networkmanager_sites
        (aws_networkmanager_sites ?id ?tags ~global_network_id ());
    attrs = __attrs;
  }
