(* DO NOT EDIT, GENERATED AUTOMATICALLY *)

open! Tf_core

type filter = {
  name : string prop;
  values : string prop list;
      [@default []] [@yojson_drop_default Stdlib.( = )]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : filter) -> ()

let yojson_of_filter =
  (function
   | { name = v_name; values = v_values } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         if Stdlib.( = ) [] v_values then bnds
         else
           let arg =
             (yojson_of_list (yojson_of_prop yojson_of_string))
               v_values
           in
           let bnd = "values", arg in
           bnd :: bnds
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_name in
         ("name", arg) :: bnds
       in
       `Assoc bnds
    : filter -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_filter

[@@@deriving.end]

type aws_ec2_transit_gateway_attachment = {
  id : string prop option; [@option]
  tags : (string * string prop) list option; [@option]
  transit_gateway_attachment_id : string prop option; [@option]
  filter : filter list;
      [@default []] [@yojson_drop_default Stdlib.( = )]
}
[@@deriving_inline yojson_of]

let _ = fun (_ : aws_ec2_transit_gateway_attachment) -> ()

let yojson_of_aws_ec2_transit_gateway_attachment =
  (function
   | {
       id = v_id;
       tags = v_tags;
       transit_gateway_attachment_id =
         v_transit_gateway_attachment_id;
       filter = v_filter;
     } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         if Stdlib.( = ) [] v_filter then bnds
         else
           let arg = (yojson_of_list yojson_of_filter) v_filter in
           let bnd = "filter", arg in
           bnd :: bnds
       in
       let bnds =
         match v_transit_gateway_attachment_id with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "transit_gateway_attachment_id", arg in
             bnd :: bnds
       in
       let bnds =
         match v_tags with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg =
               yojson_of_list
                 (function
                   | v0, v1 ->
                       let v0 = yojson_of_string v0
                       and v1 = yojson_of_prop yojson_of_string v1 in
                       `List [ v0; v1 ])
                 v
             in
             let bnd = "tags", arg in
             bnd :: bnds
       in
       let bnds =
         match v_id with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "id", arg in
             bnd :: bnds
       in
       `Assoc bnds
    : aws_ec2_transit_gateway_attachment ->
      Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_ec2_transit_gateway_attachment

[@@@deriving.end]

let filter ~name ~values () : filter = { name; values }

let aws_ec2_transit_gateway_attachment ?id ?tags
    ?transit_gateway_attachment_id ~filter () :
    aws_ec2_transit_gateway_attachment =
  { id; tags; transit_gateway_attachment_id; filter }

type t = {
  tf_name : string;
  arn : string prop;
  association_state : string prop;
  association_transit_gateway_route_table_id : string prop;
  id : string prop;
  resource_id : string prop;
  resource_owner_id : string prop;
  resource_type : string prop;
  state : string prop;
  tags : (string * string) list prop;
  transit_gateway_attachment_id : string prop;
  transit_gateway_id : string prop;
  transit_gateway_owner_id : string prop;
}

let make ?id ?tags ?transit_gateway_attachment_id ~filter __id =
  let __type = "aws_ec2_transit_gateway_attachment" in
  let __attrs =
    ({
       tf_name = __id;
       arn = Prop.computed __type __id "arn";
       association_state =
         Prop.computed __type __id "association_state";
       association_transit_gateway_route_table_id =
         Prop.computed __type __id
           "association_transit_gateway_route_table_id";
       id = Prop.computed __type __id "id";
       resource_id = Prop.computed __type __id "resource_id";
       resource_owner_id =
         Prop.computed __type __id "resource_owner_id";
       resource_type = Prop.computed __type __id "resource_type";
       state = Prop.computed __type __id "state";
       tags = Prop.computed __type __id "tags";
       transit_gateway_attachment_id =
         Prop.computed __type __id "transit_gateway_attachment_id";
       transit_gateway_id =
         Prop.computed __type __id "transit_gateway_id";
       transit_gateway_owner_id =
         Prop.computed __type __id "transit_gateway_owner_id";
     }
      : t)
  in
  {
    Tf_core.id = __id;
    type_ = __type;
    json =
      yojson_of_aws_ec2_transit_gateway_attachment
        (aws_ec2_transit_gateway_attachment ?id ?tags
           ?transit_gateway_attachment_id ~filter ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?tags ?transit_gateway_attachment_id
    ~filter __id =
  let (r : _ Tf_core.resource) =
    make ?id ?tags ?transit_gateway_attachment_id ~filter __id
  in
  Data.add ?tf_module ~type_:r.type_ ~id:r.id r.json;
  r.attrs
