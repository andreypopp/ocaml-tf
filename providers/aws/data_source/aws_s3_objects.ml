...
let _ = fun (_ : aws_s3_objects) -> ()

let yojson_of_aws_s3_objects =
  (function
   | {
       bucket = v_bucket;
       ...
       in
       `Assoc bnds
    : aws_s3_objects -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_aws_s3_objects

[@@@deriving.end]

let aws_s3_objects ?delimiter ?encoding_type ?fetch_owner ?id
    ?max_keys ?prefix ?request_payer ?start_after ~bucket () :
    aws_s3_objects =
  {
    bucket;
    delimiter;
...
let make ?delimiter ?encoding_type ?fetch_owner ?id ?max_keys ?prefix
    ?request_payer ?start_after ~bucket __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_aws_s3_objects
        (aws_s3_objects ?delimiter ?encoding_type ?fetch_owner ?id
           ?max_keys ?prefix ?request_payer ?start_after ~bucket ());
    attrs = __attrs;
  }

let register ?tf_module ?delimiter ?encoding_type ?fetch_owner ?id
    ?max_keys ?prefix ?request_payer ?start_after ~bucket __id =
