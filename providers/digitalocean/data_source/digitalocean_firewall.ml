...
let yojson_of_digitalocean_firewall =
  (function
   | {
       ...
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_firewall_id in
...
let digitalocean_firewall ?droplet_ids ?id ?tags ~firewall_id
    ~inbound_rule ~outbound_rule () : digitalocean_firewall =
...
let make ?droplet_ids ?id ?tags ~firewall_id ~inbound_rule
    ~outbound_rule __id =
  ...
  in
  {
    ...
    json =
      yojson_of_digitalocean_firewall
        (digitalocean_firewall ?droplet_ids ?id ?tags ~firewall_id
           ~inbound_rule ~outbound_rule ());
    attrs = __attrs;
  }

let register ?tf_module ?droplet_ids ?id ?tags ~firewall_id
    ~inbound_rule ~outbound_rule __id =
  let (r : _ Tf_core.resource) =
    make ?droplet_ids ?id ?tags ~firewall_id ~inbound_rule
      ~outbound_rule __id
  in
