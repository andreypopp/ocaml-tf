...
let _ = fun (_ : digitalocean_reserved_ip) -> ()

let yojson_of_digitalocean_reserved_ip =
  (function
   | { id = v_id; ip_address = v_ip_address } ->
       ...
       in
       `Assoc bnds
    : digitalocean_reserved_ip -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_digitalocean_reserved_ip

[@@@deriving.end]

let digitalocean_reserved_ip ?id ~ip_address () :
  ...
  in
  {
    ...
    json =
      yojson_of_digitalocean_reserved_ip
        (digitalocean_reserved_ip ?id ~ip_address ());
    attrs = __attrs;
  }
