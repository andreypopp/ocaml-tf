...
let _ =
 fun (_ : google_bigquery_analytics_hub_data_exchange_iam_member) ->
  ()

let yojson_of_google_bigquery_analytics_hub_data_exchange_iam_member
    =
  (function
   | {
...
let google_bigquery_analytics_hub_data_exchange_iam_member ?id
    ?location ?project ?(condition = []) ~data_exchange_id ~member
    ~role () : google_bigquery_analytics_hub_data_exchange_iam_member
    =
  {
    data_exchange_id;
    id;
...
let register ?tf_module ?id ?location ?project ?(condition = [])
    ~data_exchange_id ~member ~role __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~condition ~data_exchange_id ~member
      ~role __id
  in
