...
let yojson_of_google_folder_organization_policy =
  (function
   | { constraint_ = v_constraint_; folder = v_folder; id = v_id } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
...
let google_folder_organization_policy ?id ~constraint_ ~folder () :
    google_folder_organization_policy =
...
let make ?id ~constraint_ ~folder __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_folder_organization_policy
        (google_folder_organization_policy ?id ~constraint_ ~folder
           ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~constraint_ ~folder __id =
  let (r : _ Tf_core.resource) =
    make ?id ~constraint_ ~folder __id
  in
