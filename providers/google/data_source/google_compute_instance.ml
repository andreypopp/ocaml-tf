...
let yojson_of_boot_disk__initialize_params =
  (function
   | {
       ...
       let bnds =
         let arg =
           yojson_of_prop yojson_of_float v_provisioned_throughput
         in
...
let google_compute_instance ?id ?name ?project ?self_link ?zone () :
...
let make ?id ?name ?project ?self_link ?zone __id =
  ...
  let __attrs =
    ({
       ...
       boot_disk = Prop.computed __type __id "boot_disk";
  ...
  in
  {
    ...
    json =
      yojson_of_google_compute_instance
        (google_compute_instance ?id ?name ?project ?self_link ?zone
           ());
    attrs = __attrs;
  }
