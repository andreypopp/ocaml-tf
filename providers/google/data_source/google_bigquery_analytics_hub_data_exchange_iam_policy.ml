...
let google_bigquery_analytics_hub_data_exchange_iam_policy ?id
    ?location ?project ~data_exchange_id () :
    google_bigquery_analytics_hub_data_exchange_iam_policy =
  { data_exchange_id; id; location; project }
...
let make ?id ?location ?project ~data_exchange_id __id =
  let __type =
    "google_bigquery_analytics_hub_data_exchange_iam_policy"
  in
  let __attrs =
...
let register ?tf_module ?id ?location ?project ~data_exchange_id __id
    =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~data_exchange_id __id
  in
