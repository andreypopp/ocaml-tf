...
let yojson_of_disk =
  (function
   | {
       ...
       in
       let bnds =
         if [] = v_resource_policies then bnds
         else
           let arg =
...
let _ = fun (_ : google_compute_region_instance_template) -> ()

let yojson_of_google_compute_region_instance_template =
  (function
   | {
       filter = v_filter;
       id = v_id;
...
let _ = yojson_of_google_compute_region_instance_template

[@@@deriving.end]

let google_compute_region_instance_template ?filter ?id ?most_recent
    ?name ?project ?region () :
    google_compute_region_instance_template =
...
let make ?filter ?id ?most_recent ?name ?project ?region __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_compute_region_instance_template
        (google_compute_region_instance_template ?filter ?id
           ?most_recent ?name ?project ?region ());
    attrs = __attrs;
  }
...
let register ?tf_module ?filter ?id ?most_recent ?name ?project
  ...
  let (r : _ Tf_core.resource) =
    make ?filter ?id ?most_recent ?name ?project ?region __id
  in
