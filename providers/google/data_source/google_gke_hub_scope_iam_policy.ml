...
let yojson_of_google_gke_hub_scope_iam_policy =
  (function
   | { id = v_id; project = v_project; scope_id = v_scope_id } ->
...
let google_gke_hub_scope_iam_policy ?id ?project ~scope_id () :
    google_gke_hub_scope_iam_policy =
  { id; project; scope_id }
...
let make ?id ?project ~scope_id __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_gke_hub_scope_iam_policy
        (google_gke_hub_scope_iam_policy ?id ?project ~scope_id ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?project ~scope_id __id =
  let (r : _ Tf_core.resource) = make ?id ?project ~scope_id __id in
