...
let yojson_of_google_compute_image_iam_policy =
  (function
   | { id = v_id; image = v_image; project = v_project } ->
...
let google_compute_image_iam_policy ?id ?project ~image () :
    google_compute_image_iam_policy =
  { id; image; project }
...
let make ?id ?project ~image __id =
...
let register ?tf_module ?id ?project ~image __id =
  let (r : _ Tf_core.resource) = make ?id ?project ~image __id in
