...
let _ =
 fun (_ : google_secure_source_manager_instance_iam_policy) -> ()

let yojson_of_google_secure_source_manager_instance_iam_policy =
  (function
   | {
       id = v_id;
...
let _ = yojson_of_google_secure_source_manager_instance_iam_policy
...
let google_secure_source_manager_instance_iam_policy ?id ?location
    ?project ~instance_id () :
    google_secure_source_manager_instance_iam_policy =
  { id; instance_id; location; project }
...
let make ?id ?location ?project ~instance_id __id =
    ...
    json =
      yojson_of_google_secure_source_manager_instance_iam_policy
        (google_secure_source_manager_instance_iam_policy ?id
           ?location ?project ~instance_id ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ~instance_id __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~instance_id __id
  in
