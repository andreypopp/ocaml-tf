...
let yojson_of_google_healthcare_fhir_store_iam_policy =
  (function
   | { fhir_store_id = v_fhir_store_id; id = v_id } ->
...
let make ?id ~fhir_store_id __id =
...
let register ?tf_module ?id ~fhir_store_id __id =
  let (r : _ Tf_core.resource) = make ?id ~fhir_store_id __id in
