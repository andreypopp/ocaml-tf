...
let _ = fun (_ : google_pubsub_subscription_iam_policy) -> ()

let yojson_of_google_pubsub_subscription_iam_policy =
  (function
   | {
       id = v_id;
       project = v_project;
...
let _ = yojson_of_google_pubsub_subscription_iam_policy

[@@@deriving.end]

let google_pubsub_subscription_iam_policy ?id ?project ~subscription
    () : google_pubsub_subscription_iam_policy =
...
let make ?id ?project ~subscription __id =
...
let register ?tf_module ?id ?project ~subscription __id =
  let (r : _ Tf_core.resource) =
    make ?id ?project ~subscription __id
  in
