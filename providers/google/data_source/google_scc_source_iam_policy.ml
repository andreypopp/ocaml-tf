...
let yojson_of_google_scc_source_iam_policy =
  (function
   | { id = v_id; organization = v_organization; source = v_source }
     ->
...
let google_scc_source_iam_policy ?id ~organization ~source () :
    google_scc_source_iam_policy =
...
let make ?id ~organization ~source __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_scc_source_iam_policy
        (google_scc_source_iam_policy ?id ~organization ~source ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~organization ~source __id =
  let (r : _ Tf_core.resource) =
    make ?id ~organization ~source __id
  in
