...
let _ = fun (_ : google_dataplex_datascan_iam_policy) -> ()

let yojson_of_google_dataplex_datascan_iam_policy =
  (function
   | {
...
let _ = yojson_of_google_dataplex_datascan_iam_policy

[@@@deriving.end]

let google_dataplex_datascan_iam_policy ?id ?location ?project
    ~data_scan_id () : google_dataplex_datascan_iam_policy =
...
let make ?id ?location ?project ~data_scan_id __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_dataplex_datascan_iam_policy
        (google_dataplex_datascan_iam_policy ?id ?location ?project
           ~data_scan_id ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ~data_scan_id __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~data_scan_id __id
  in
