...
let yojson_of_google_apigee_environment_iam_policy =
  (function
   | { env_id = v_env_id; id = v_id; org_id = v_org_id } ->
...
let make ?id ~env_id ~org_id __id =
...
let register ?tf_module ?id ~env_id ~org_id __id =
  let (r : _ Tf_core.resource) = make ?id ~env_id ~org_id __id in
