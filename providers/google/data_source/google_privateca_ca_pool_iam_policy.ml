...
let _ = fun (_ : google_privateca_ca_pool_iam_policy) -> ()

let yojson_of_google_privateca_ca_pool_iam_policy =
  (function
   | {
       ca_pool = v_ca_pool;
       id = v_id;
...
let _ = yojson_of_google_privateca_ca_pool_iam_policy

[@@@deriving.end]

let google_privateca_ca_pool_iam_policy ?id ?location ?project
    ~ca_pool () : google_privateca_ca_pool_iam_policy =
...
let make ?id ?location ?project ~ca_pool __id =
...
let register ?tf_module ?id ?location ?project ~ca_pool __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~ca_pool __id
  in
