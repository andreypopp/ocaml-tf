...
let yojson_of_google_data_catalog_policy_tag_iam_policy =
  (function
   | { id = v_id; policy_tag = v_policy_tag } ->
...
let google_data_catalog_policy_tag_iam_policy ?id ~policy_tag () :
    google_data_catalog_policy_tag_iam_policy =
  { id; policy_tag }
  ...
  in
  {
    ...
    json =
      yojson_of_google_data_catalog_policy_tag_iam_policy
        (google_data_catalog_policy_tag_iam_policy ?id ~policy_tag ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~policy_tag __id =
  let (r : _ Tf_core.resource) = make ?id ~policy_tag __id in
