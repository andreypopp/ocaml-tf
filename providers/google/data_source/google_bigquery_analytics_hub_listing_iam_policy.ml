...
let _ =
 fun (_ : google_bigquery_analytics_hub_listing_iam_policy) -> ()

let yojson_of_google_bigquery_analytics_hub_listing_iam_policy =
  (function
   | {
...
let _ = yojson_of_google_bigquery_analytics_hub_listing_iam_policy

[@@@deriving.end]

let google_bigquery_analytics_hub_listing_iam_policy ?id ?location
    ?project ~data_exchange_id ~listing_id () :
    google_bigquery_analytics_hub_listing_iam_policy =
  { data_exchange_id; id; listing_id; location; project }
...
let make ?id ?location ?project ~data_exchange_id ~listing_id __id =
...
let register ?tf_module ?id ?location ?project ~data_exchange_id
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~data_exchange_id ~listing_id __id
  in
