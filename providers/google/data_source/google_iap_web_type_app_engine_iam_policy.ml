...
let _ = fun (_ : google_iap_web_type_app_engine_iam_policy) -> ()

let yojson_of_google_iap_web_type_app_engine_iam_policy =
  (function
   | { app_id = v_app_id; id = v_id; project = v_project } ->
...
let _ = yojson_of_google_iap_web_type_app_engine_iam_policy
...
let make ?id ?project ~app_id __id =
...
let register ?tf_module ?id ?project ~app_id __id =
  let (r : _ Tf_core.resource) = make ?id ?project ~app_id __id in
