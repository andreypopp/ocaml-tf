...
let yojson_of_google_healthcare_dicom_store_iam_policy =
  (function
   | { dicom_store_id = v_dicom_store_id; id = v_id } ->
...
let make ?id ~dicom_store_id __id =
...
let register ?tf_module ?id ~dicom_store_id __id =
  let (r : _ Tf_core.resource) = make ?id ~dicom_store_id __id in
