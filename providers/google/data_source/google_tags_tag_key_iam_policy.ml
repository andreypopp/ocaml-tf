...
let google_tags_tag_key_iam_policy ?id ~tag_key () :
    google_tags_tag_key_iam_policy =
  { id; tag_key }
  ...
  in
  {
    ...
    json =
      yojson_of_google_tags_tag_key_iam_policy
        (google_tags_tag_key_iam_policy ?id ~tag_key ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~tag_key __id =
  let (r : _ Tf_core.resource) = make ?id ~tag_key __id in
