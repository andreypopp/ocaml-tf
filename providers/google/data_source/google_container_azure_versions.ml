...
let _ = fun (_ : google_container_azure_versions) -> ()

let yojson_of_google_container_azure_versions =
  (function
...
let _ = yojson_of_google_container_azure_versions

[@@@deriving.end]

let google_container_azure_versions ?id ?location ?project () :
    google_container_azure_versions =
  { id; location; project }
...
let make ?id ?location ?project __id =
  let __type = "google_container_azure_versions" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_container_azure_versions
        (google_container_azure_versions ?id ?location ?project ());
    attrs = __attrs;
  }
