...
let _ = fun (_ : google_notebooks_instance_iam_policy) -> ()

let yojson_of_google_notebooks_instance_iam_policy =
  (function
   | {
       id = v_id;
...
let _ = yojson_of_google_notebooks_instance_iam_policy

[@@@deriving.end]

let google_notebooks_instance_iam_policy ?id ?location ?project
    ~instance_name () : google_notebooks_instance_iam_policy =
  { id; instance_name; location; project }
...
let make ?id ?location ?project ~instance_name __id =
...
let register ?tf_module ?id ?location ?project ~instance_name __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~instance_name __id
  in
