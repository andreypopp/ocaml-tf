...
let yojson_of_google_service_account_iam_policy =
  (function
   | { id = v_id; service_account_id = v_service_account_id } ->
...
let google_service_account_iam_policy ?id ~service_account_id () :
    google_service_account_iam_policy =
...
let make ?id ~service_account_id __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_service_account_iam_policy
        (google_service_account_iam_policy ?id ~service_account_id ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~service_account_id __id =
  let (r : _ Tf_core.resource) = make ?id ~service_account_id __id in
