...
let yojson_of_google_storage_bucket_object =
  (function
   | { bucket = v_bucket; id = v_id; name = v_name } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
...
let google_storage_bucket_object ?bucket ?id ?name () :
    google_storage_bucket_object =
  { bucket; id; name }
  ...
  in
  {
    ...
    json =
      yojson_of_google_storage_bucket_object
        (google_storage_bucket_object ?bucket ?id ?name ());
    attrs = __attrs;
  }

let register ?tf_module ?bucket ?id ?name __id =
  let (r : _ Tf_core.resource) = make ?bucket ?id ?name __id in
