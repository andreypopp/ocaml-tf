...
let google_organization_iam_policy ?id ~org_id () :
    google_organization_iam_policy =
  { id; org_id }
  ...
  in
  {
    ...
    json =
      yojson_of_google_organization_iam_policy
        (google_organization_iam_policy ?id ~org_id ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~org_id __id =
  let (r : _ Tf_core.resource) = make ?id ~org_id __id in
