...
let google_cloudfunctions2_function_iam_policy ?id ?location ?project
    ~cloud_function () : google_cloudfunctions2_function_iam_policy =
  { cloud_function; id; location; project }
...
let make ?id ?location ?project ~cloud_function __id =
    ...
    json =
      yojson_of_google_cloudfunctions2_function_iam_policy
        (google_cloudfunctions2_function_iam_policy ?id ?location
           ?project ~cloud_function ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ~cloud_function __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~cloud_function __id
  in
