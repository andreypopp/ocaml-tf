...
let make ?id ?project ?region ~cluster __id =
...
let register ?tf_module ?id ?project ?region ~cluster __id =
  let (r : _ Tf_core.resource) =
    make ?id ?project ?region ~cluster __id
  in
