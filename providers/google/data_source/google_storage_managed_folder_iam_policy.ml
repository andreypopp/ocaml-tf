...
let google_storage_managed_folder_iam_policy ?id ~bucket
    ~managed_folder () : google_storage_managed_folder_iam_policy =
  { bucket; id; managed_folder }
...
let make ?id ~bucket ~managed_folder __id =
...
let register ?tf_module ?id ~bucket ~managed_folder __id =
  let (r : _ Tf_core.resource) =
    make ?id ~bucket ~managed_folder __id
  in
