...
let _ = fun (_ : google_iap_tunnel_instance_iam_binding) -> ()

let yojson_of_google_iap_tunnel_instance_iam_binding =
  (function
   | {
       id = v_id;
       instance = v_instance;
...
let _ = yojson_of_google_iap_tunnel_instance_iam_binding
...
let make ?id ?project ?zone ?(condition = []) ~instance ~members
    ~role __id =
...
let register ?tf_module ?id ?project ?zone ?(condition = [])
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?project ?zone ~condition ~instance ~members ~role __id
  in
