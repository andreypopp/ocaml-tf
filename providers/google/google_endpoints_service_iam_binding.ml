...
let _ = fun (_ : google_endpoints_service_iam_binding) -> ()

let yojson_of_google_endpoints_service_iam_binding =
  (function
   | {
       id = v_id;
       members = v_members;
...
let _ = yojson_of_google_endpoints_service_iam_binding
...
let google_endpoints_service_iam_binding ?id ?(condition = [])
    ~members ~role ~service_name () :
    google_endpoints_service_iam_binding =
...
let make ?id ?(condition = []) ~members ~role ~service_name __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_endpoints_service_iam_binding
        (google_endpoints_service_iam_binding ?id ~condition ~members
           ~role ~service_name ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?(condition = []) ~members ~role
  ...
  let (r : _ Tf_core.resource) =
    make ?id ~condition ~members ~role ~service_name __id
  in
