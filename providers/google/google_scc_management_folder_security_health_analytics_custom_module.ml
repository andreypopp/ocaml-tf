...
type google_scc_management_folder_security_health_analytics_custom_module = {
  display_name : string prop option; [@option]
  enablement_state : string prop option; [@option]
  folder : string prop;
...
let yojson_of_google_scc_management_folder_security_health_analytics_custom_module
    =
  (function
   | {
       ...
       in
       let bnds =
         match v_enablement_state with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "enablement_state", arg in
             bnd :: bnds
       in
       let bnds =
...
let _ =
  yojson_of_google_scc_management_folder_security_health_analytics_custom_module
...
let custom_config ?description ?recommendation ?severity
    ?(custom_output = []) ?(predicate = []) ?(resource_selector = [])
    () : custom_config =
  {
    description;
    recommendation;
...
let google_scc_management_folder_security_health_analytics_custom_module
    ?display_name ?enablement_state ?id ?location
    ?(custom_config = []) ?timeouts ~folder () :
    google_scc_management_folder_security_health_analytics_custom_module
    =
  {
    display_name;
...
let make ?display_name ?enablement_state ?id ?location
    ?(custom_config = []) ?timeouts ~folder __id =
  let __type =
    "google_scc_management_folder_security_health_analytics_custom_module"
  in
  let __attrs =
...
let register ?tf_module ?display_name ?enablement_state ?id ?location
    ?(custom_config = []) ?timeouts ~folder __id =
  let (r : _ Tf_core.resource) =
    make ?display_name ?enablement_state ?id ?location ~custom_config
