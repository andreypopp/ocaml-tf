...
let _ = fun (_ : google_gke_hub_feature_iam_member) -> ()

let yojson_of_google_gke_hub_feature_iam_member =
  (function
   | {
       id = v_id;
       location = v_location;
...
let _ = yojson_of_google_gke_hub_feature_iam_member
...
let google_gke_hub_feature_iam_member ?id ?location ?project
...
let make ?id ?location ?project ?(condition = []) ~member ~name ~role
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_gke_hub_feature_iam_member
        (google_gke_hub_feature_iam_member ?id ?location ?project
           ~condition ~member ~name ~role ());
    attrs = __attrs;
  }
