...
let _ = fun (_ : google_tags_tag_key_iam_binding) -> ()

let yojson_of_google_tags_tag_key_iam_binding =
  (function
   | {
       id = v_id;
       members = v_members;
...
let _ = yojson_of_google_tags_tag_key_iam_binding
...
let google_tags_tag_key_iam_binding ?id ?(condition = []) ~members
    ~role ~tag_key () : google_tags_tag_key_iam_binding =
...
let make ?id ?(condition = []) ~members ~role ~tag_key __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_tags_tag_key_iam_binding
        (google_tags_tag_key_iam_binding ?id ~condition ~members
           ~role ~tag_key ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?(condition = []) ~members ~role ~tag_key
    __id =
  let (r : _ Tf_core.resource) =
    make ?id ~condition ~members ~role ~tag_key __id
  in
