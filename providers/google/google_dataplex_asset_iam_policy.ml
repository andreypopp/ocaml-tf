...
let make ?id ?location ?project ~asset ~dataplex_zone ~lake
    ~policy_data __id =
    ...
    json =
      yojson_of_google_dataplex_asset_iam_policy
        (google_dataplex_asset_iam_policy ?id ?location ?project
           ~asset ~dataplex_zone ~lake ~policy_data ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ~asset ~dataplex_zone
    ~lake ~policy_data __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~asset ~dataplex_zone ~lake
      ~policy_data __id
  in
