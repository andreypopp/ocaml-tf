...
let _ = fun (_ : google_iap_web_iam_binding) -> ()

let yojson_of_google_iap_web_iam_binding =
  (function
   | {
       id = v_id;
       members = v_members;
       ...
       in
       `Assoc bnds
    : google_iap_web_iam_binding -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_google_iap_web_iam_binding
...
let google_iap_web_iam_binding ?id ?project ?(condition = [])
    ~members ~role () : google_iap_web_iam_binding =
...
let make ?id ?project ?(condition = []) ~members ~role __id =
  let __type = "google_iap_web_iam_binding" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_iap_web_iam_binding
        (google_iap_web_iam_binding ?id ?project ~condition ~members
           ~role ());
    attrs = __attrs;
  }
