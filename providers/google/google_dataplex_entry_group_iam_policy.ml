...
let google_dataplex_entry_group_iam_policy ?id ?location ?project
    ...
    google_dataplex_entry_group_iam_policy =
  { entry_group_id; id; location; policy_data; project }
...
let make ?id ?location ?project ~entry_group_id ~policy_data __id =
    ...
    json =
      yojson_of_google_dataplex_entry_group_iam_policy
        (google_dataplex_entry_group_iam_policy ?id ?location
           ?project ~entry_group_id ~policy_data ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ~entry_group_id
    ~policy_data __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~entry_group_id ~policy_data __id
  in
