...
let _ = fun (_ : google_compute_instance_iam_member) -> ()

let yojson_of_google_compute_instance_iam_member =
  (function
   | {
       id = v_id;
...
let _ = yojson_of_google_compute_instance_iam_member
...
let make ?id ?project ?zone ?(condition = []) ~instance_name ~member
    ~role __id =
...
let register ?tf_module ?id ?project ?zone ?(condition = [])
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?project ?zone ~condition ~instance_name ~member ~role
      __id
  in
