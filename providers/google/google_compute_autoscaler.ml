...
let google_compute_autoscaler ?description ?id ?project ?zone
    ?timeouts ~name ~target ~autoscaling_policy () :
...
let make ?description ?id ?project ?zone ?timeouts ~name ~target
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_compute_autoscaler
        (google_compute_autoscaler ?description ?id ?project ?zone
           ?timeouts ~name ~target ~autoscaling_policy ());
    attrs = __attrs;
  }

let register ?tf_module ?description ?id ?project ?zone ?timeouts
    ~name ~target ~autoscaling_policy __id =
  let (r : _ Tf_core.resource) =
    make ?description ?id ?project ?zone ?timeouts ~name ~target
      ~autoscaling_policy __id
  in
