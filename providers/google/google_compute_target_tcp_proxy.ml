...
let _ = fun (_ : google_compute_target_tcp_proxy) -> ()

let yojson_of_google_compute_target_tcp_proxy =
  (function
   | {
...
let _ = yojson_of_google_compute_target_tcp_proxy
...
let make ?description ?id ?project ?proxy_bind ?proxy_header
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_compute_target_tcp_proxy
...
let register ?tf_module ?description ?id ?project ?proxy_bind
    ?proxy_header ?timeouts ~backend_service ~name __id =
  let (r : _ Tf_core.resource) =
    make ?description ?id ?project ?proxy_bind ?proxy_header
      ?timeouts ~backend_service ~name __id
  in
