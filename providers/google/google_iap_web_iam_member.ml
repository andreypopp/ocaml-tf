...
let _ = fun (_ : google_iap_web_iam_member) -> ()

let yojson_of_google_iap_web_iam_member =
  (function
   | {
       id = v_id;
       member = v_member;
       ...
       in
       `Assoc bnds
    : google_iap_web_iam_member -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_google_iap_web_iam_member
...
let google_iap_web_iam_member ?id ?project ?(condition = []) ~member
    ~role () : google_iap_web_iam_member =
  ...
  in
  {
    ...
    json =
      yojson_of_google_iap_web_iam_member
        (google_iap_web_iam_member ?id ?project ~condition ~member
           ~role ());
    attrs = __attrs;
  }
