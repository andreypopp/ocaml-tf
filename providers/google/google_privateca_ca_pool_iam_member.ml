...
let make ?id ?location ?project ?(condition = []) ~ca_pool ~member
    ~role __id =
...
let register ?tf_module ?id ?location ?project ?(condition = [])
    ~ca_pool ~member ~role __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~condition ~ca_pool ~member ~role
      __id
  in
