...
let _ = fun (_ : google_privateca_ca_pool_iam_member) -> ()

let yojson_of_google_privateca_ca_pool_iam_member =
  (function
   | {
       ca_pool = v_ca_pool;
       id = v_id;
...
let _ = yojson_of_google_privateca_ca_pool_iam_member
...
let make ?id ?location ?project ~ca_pool ~member ~role ~condition
    __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_privateca_ca_pool_iam_member
        (google_privateca_ca_pool_iam_member ?id ?location ?project
           ~ca_pool ~member ~role ~condition ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ~ca_pool ~member ~role
    ~condition __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~ca_pool ~member ~role ~condition
      __id
  in
