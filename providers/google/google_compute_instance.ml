...
let _ = fun (_ : google_compute_instance) -> ()

let yojson_of_google_compute_instance =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_scratch_disk then bnds
         else
           let arg =
             (yojson_of_list yojson_of_scratch_disk) v_scratch_disk
           in
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_boot_disk then bnds
         else
           let arg =
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_attached_disk then bnds
         else
           let arg =
             (yojson_of_list yojson_of_attached_disk) v_attached_disk
           in
       ...
       in
       `Assoc bnds
    : google_compute_instance -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_google_compute_instance
