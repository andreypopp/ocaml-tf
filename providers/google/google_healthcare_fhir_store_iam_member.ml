...
let make ?id ~fhir_store_id ~member ~role ~condition __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_healthcare_fhir_store_iam_member
        (google_healthcare_fhir_store_iam_member ?id ~fhir_store_id
           ~member ~role ~condition ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~fhir_store_id ~member ~role ~condition
    __id =
  let (r : _ Tf_core.resource) =
    make ?id ~fhir_store_id ~member ~role ~condition __id
  in
