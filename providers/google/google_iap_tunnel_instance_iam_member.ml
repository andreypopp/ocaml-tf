...
let _ = fun (_ : google_iap_tunnel_instance_iam_member) -> ()

let yojson_of_google_iap_tunnel_instance_iam_member =
  (function
   | {
       id = v_id;
       instance = v_instance;
...
let _ = yojson_of_google_iap_tunnel_instance_iam_member
...
let make ?id ?project ?zone ?(condition = []) ~instance ~member ~role
    __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_iap_tunnel_instance_iam_member
...
let register ?tf_module ?id ?project ?zone ?(condition = [])
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?project ?zone ~condition ~instance ~member ~role __id
  in
