...
let _ = fun (_ : google_scc_project_notification_config) -> ()

let yojson_of_google_scc_project_notification_config =
  (function
   | {
...
let _ = yojson_of_google_scc_project_notification_config
...
let google_scc_project_notification_config ?description ?id ?project
    ?timeouts ~config_id ~pubsub_topic ~streaming_config () :
    google_scc_project_notification_config =
  {
    config_id;
    description;
...
let make ?description ?id ?project ?timeouts ~config_id ~pubsub_topic
    ~streaming_config __id =
  let __type = "google_scc_project_notification_config" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_scc_project_notification_config
        (google_scc_project_notification_config ?description ?id
           ?project ?timeouts ~config_id ~pubsub_topic
           ~streaming_config ());
    attrs = __attrs;
  }

let register ?tf_module ?description ?id ?project ?timeouts
    ~config_id ~pubsub_topic ~streaming_config __id =
  let (r : _ Tf_core.resource) =
    make ?description ?id ?project ?timeouts ~config_id ~pubsub_topic
      ~streaming_config __id
  in
