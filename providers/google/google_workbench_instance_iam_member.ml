...
let register ?tf_module ?id ?location ?project ~member ~name ~role ~condition __resource_id =
  let __resource_type = "google_workbench_instance_iam_member" in
  let __resource = google_workbench_instance_iam_member ?id ?location ?project ~member ~name ~role ~condition () in
