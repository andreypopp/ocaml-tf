...
let _ =
 fun (_ : google_bigquery_datapolicy_data_policy_iam_binding) -> ()

let yojson_of_google_bigquery_datapolicy_data_policy_iam_binding =
  (function
   | {
...
let _ = yojson_of_google_bigquery_datapolicy_data_policy_iam_binding
...
let google_bigquery_datapolicy_data_policy_iam_binding ?id ?location
    ?project ?(condition = []) ~data_policy_id ~members ~role () :
    google_bigquery_datapolicy_data_policy_iam_binding =
    ...
    json =
      yojson_of_google_bigquery_datapolicy_data_policy_iam_binding
        (google_bigquery_datapolicy_data_policy_iam_binding ?id
           ?location ?project ~condition ~data_policy_id ~members
           ~role ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ?(condition = [])
    ~data_policy_id ~members ~role __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~condition ~data_policy_id ~members
      ~role __id
  in
