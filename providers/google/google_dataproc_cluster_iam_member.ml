...
let _ = fun (_ : google_dataproc_cluster_iam_member) -> ()

let yojson_of_google_dataproc_cluster_iam_member =
  (function
   | {
       cluster = v_cluster;
       id = v_id;
...
let _ = yojson_of_google_dataproc_cluster_iam_member
...
let google_dataproc_cluster_iam_member ?id ?project ?region
    ?(condition = []) ~cluster ~member ~role () :
    google_dataproc_cluster_iam_member =
...
let make ?id ?project ?region ?(condition = []) ~cluster ~member
    ~role __id =
...
let register ?tf_module ?id ?project ?region ?(condition = [])
    ~cluster ~member ~role __id =
  let (r : _ Tf_core.resource) =
    make ?id ?project ?region ~condition ~cluster ~member ~role __id
  in
