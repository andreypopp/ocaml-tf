...
let yojson_of_virtual_machine__virtual_machine_config =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_container_images then bnds
         else
           let arg =
             (yojson_of_list
                yojson_of_virtual_machine__virtual_machine_config__container_images)
...
let yojson_of_virtual_machine =
  (function
   | { virtual_machine_config = v_virtual_machine_config } ->
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_virtual_machine_config then bnds
         else
           let arg =
             (yojson_of_list
...
type metrics = { system_metrics : string prop Tf_core.assoc }
[@@deriving_inline yojson_of]
