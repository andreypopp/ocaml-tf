...
let google_secure_source_manager_repository_iam_policy ?id ?location
    ?project ~policy_data ~repository_id () :
    google_secure_source_manager_repository_iam_policy =
  { id; location; policy_data; project; repository_id }
...
let make ?id ?location ?project ~policy_data ~repository_id __id =
  let __type =
    "google_secure_source_manager_repository_iam_policy"
  in
  let __attrs =
    ...
    json =
      yojson_of_google_secure_source_manager_repository_iam_policy
        (google_secure_source_manager_repository_iam_policy ?id
           ?location ?project ~policy_data ~repository_id ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?location ?project ~policy_data
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~policy_data ~repository_id __id
  in
