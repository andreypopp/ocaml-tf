...
let yojson_of_google_privateca_certificate_authority =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_subordinate_config then bnds
         else
           let arg =
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_key_spec then bnds
         else
           let arg =
...
let config ?(subject_key_id = []) ~subject_config ~x509_config () :
    config =
  { subject_config; subject_key_id; x509_config }
