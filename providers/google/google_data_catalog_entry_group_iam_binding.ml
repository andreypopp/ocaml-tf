...
let google_data_catalog_entry_group_iam_binding ?id ?project ?region
    ?(condition = []) ~entry_group ~members ~role () :
    google_data_catalog_entry_group_iam_binding =
...
let make ?id ?project ?region ?(condition = []) ~entry_group ~members
    ~role __id =
    ...
    json =
      yojson_of_google_data_catalog_entry_group_iam_binding
        (google_data_catalog_entry_group_iam_binding ?id ?project
           ?region ~condition ~entry_group ~members ~role ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?project ?region ?(condition = [])
    ~entry_group ~members ~role __id =
  let (r : _ Tf_core.resource) =
    make ?id ?project ?region ~condition ~entry_group ~members ~role
      __id
  in
