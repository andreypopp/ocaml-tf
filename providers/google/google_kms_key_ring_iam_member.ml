...
let google_kms_key_ring_iam_member ?id ~key_ring_id ~member ~role
    ~condition () : google_kms_key_ring_iam_member =
...
let make ?id ~key_ring_id ~member ~role ~condition __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_kms_key_ring_iam_member
        (google_kms_key_ring_iam_member ?id ~key_ring_id ~member
           ~role ~condition ());
    attrs = __attrs;
  }

let register ?tf_module ?id ~key_ring_id ~member ~role ~condition
    __id =
  let (r : _ Tf_core.resource) =
    make ?id ~key_ring_id ~member ~role ~condition __id
  in
