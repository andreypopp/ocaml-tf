...
let register ?tf_module ?id ?location ?project ~members ~name ~role ~condition __resource_id =
  let __resource_type = "google_workbench_instance_iam_binding" in
  let __resource = google_workbench_instance_iam_binding ?id ?location ?project ~members ~name ~role ~condition () in
  ...
  let __resource_attributes = ({
    ...
    members = Prop.computed __resource_type __resource_id "members";
