...
let google_scc_project_custom_module ?id ?project ?timeouts
    ~display_name ~enablement_state ~custom_config () :
    google_scc_project_custom_module =
  {
    display_name;
    enablement_state;
...
let make ?id ?project ?timeouts ~display_name ~enablement_state
    ~custom_config __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_scc_project_custom_module
        (google_scc_project_custom_module ?id ?project ?timeouts
           ~display_name ~enablement_state ~custom_config ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?project ?timeouts ~display_name
    ~enablement_state ~custom_config __id =
  let (r : _ Tf_core.resource) =
    make ?id ?project ?timeouts ~display_name ~enablement_state
      ~custom_config __id
  in
