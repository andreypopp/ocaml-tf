...
let _ = fun (_ : google_billing_account_iam_member) -> ()

let yojson_of_google_billing_account_iam_member =
  (function
   | {
...
let _ = yojson_of_google_billing_account_iam_member
...
let google_billing_account_iam_member ?id ?(condition = [])
...
let make ?id ?(condition = []) ~billing_account_id ~member ~role __id
    =
  ...
  in
  {
    ...
    json =
      yojson_of_google_billing_account_iam_member
        (google_billing_account_iam_member ?id ~condition
           ~billing_account_id ~member ~role ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?(condition = []) ~billing_account_id
  ...
  let (r : _ Tf_core.resource) =
    make ?id ~condition ~billing_account_id ~member ~role __id
  in
