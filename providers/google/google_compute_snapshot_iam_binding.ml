...
let google_compute_snapshot_iam_binding ?id ?project ~members ~name
    ~role ~condition () : google_compute_snapshot_iam_binding =
...
let make ?id ?project ~members ~name ~role ~condition __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_compute_snapshot_iam_binding
        (google_compute_snapshot_iam_binding ?id ?project ~members
           ~name ~role ~condition ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?project ~members ~name ~role ~condition
    __id =
  let (r : _ Tf_core.resource) =
    make ?id ?project ~members ~name ~role ~condition __id
  in
