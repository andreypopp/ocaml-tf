...
let google_dataproc_metastore_federation_iam_policy ?id ?location
    ?project ~federation_id ~policy_data () :
    google_dataproc_metastore_federation_iam_policy =
  { federation_id; id; location; policy_data; project }
...
let make ?id ?location ?project ~federation_id ~policy_data __id =
    ...
    json =
      yojson_of_google_dataproc_metastore_federation_iam_policy
        (google_dataproc_metastore_federation_iam_policy ?id
           ?location ?project ~federation_id ~policy_data ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?location ?project ~federation_id
    ~policy_data __id =
  let (r : _ Tf_core.resource) =
    make ?id ?location ?project ~federation_id ~policy_data __id
  in
