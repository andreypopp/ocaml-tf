...
let _ = fun (_ : google_cloudfunctions_function_iam_member) -> ()

let yojson_of_google_cloudfunctions_function_iam_member =
  (function
   | {
...
let _ = yojson_of_google_cloudfunctions_function_iam_member
...
let make ?id ?project ?region ?(condition = []) ~cloud_function
    ~member ~role __id =
    ...
    json =
      yojson_of_google_cloudfunctions_function_iam_member
        (google_cloudfunctions_function_iam_member ?id ?project
           ?region ~condition ~cloud_function ~member ~role ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?project ?region ?(condition = [])
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?project ?region ~condition ~cloud_function ~member
      ~role __id
  in
