...
let google_bigtable_instance_iam_member ?id ?project
    ?(condition = []) ~instance ~member ~role () :
    google_bigtable_instance_iam_member =
...
let make ?id ?project ?(condition = []) ~instance ~member ~role __id
    =
  ...
  in
  {
    ...
    json =
      yojson_of_google_bigtable_instance_iam_member
        (google_bigtable_instance_iam_member ?id ?project ~condition
...
let register ?tf_module ?id ?project ?(condition = []) ~instance
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?project ~condition ~instance ~member ~role __id
  in
