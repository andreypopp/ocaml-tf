...
let _ = fun (_ : google_tags_tag_value_iam_member) -> ()

let yojson_of_google_tags_tag_value_iam_member =
  (function
   | {
       id = v_id;
       member = v_member;
...
let _ = yojson_of_google_tags_tag_value_iam_member
...
let google_tags_tag_value_iam_member ?id ?(condition = []) ~member
    ~role ~tag_value () : google_tags_tag_value_iam_member =
...
let make ?id ?(condition = []) ~member ~role ~tag_value __id =
  ...
  in
  {
    ...
    json =
      yojson_of_google_tags_tag_value_iam_member
        (google_tags_tag_value_iam_member ?id ~condition ~member
           ~role ~tag_value ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?(condition = []) ~member ~role
  ...
  let (r : _ Tf_core.resource) =
    make ?id ~condition ~member ~role ~tag_value __id
  in
