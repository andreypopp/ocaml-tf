...
let _ = fun (_ : google_cloud_run_v2_job_iam_binding) -> ()

let yojson_of_google_cloud_run_v2_job_iam_binding =
  (function
   | {
       id = v_id;
       location = v_location;
...
let _ = yojson_of_google_cloud_run_v2_job_iam_binding
...
let make ?id ?location ?project ?(condition = []) ~members ~name
    ~role __id =
  let __type = "google_cloud_run_v2_job_iam_binding" in
  let __attrs =
    ({
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_cloud_run_v2_job_iam_binding
        (google_cloud_run_v2_job_iam_binding ?id ?location ?project
           ~condition ~members ~name ~role ());
    attrs = __attrs;
  }
