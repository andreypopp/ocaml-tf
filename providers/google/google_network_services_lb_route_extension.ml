...
type google_network_services_lb_route_extension = {
...
let _ = fun (_ : google_network_services_lb_route_extension) -> ()

let yojson_of_google_network_services_lb_route_extension =
  (function
   | {
...
let _ = yojson_of_google_network_services_lb_route_extension

[@@@deriving.end]

let extension_chains__extensions ?authority ?fail_open
    ?forward_headers ?timeout ~name ~service () :
    extension_chains__extensions =
  { authority; fail_open; forward_headers; name; service; timeout }
...
let google_network_services_lb_route_extension ?description ?id
    ?labels ?project ?timeouts ~forwarding_rules
    ~load_balancing_scheme ~location ~name ~extension_chains () :
    google_network_services_lb_route_extension =
  {
    description;
    forwarding_rules;
...
let make ?description ?id ?labels ?project ?timeouts
    ...
    ~extension_chains __id =
  let __type = "google_network_services_lb_route_extension" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_google_network_services_lb_route_extension
        (google_network_services_lb_route_extension ?description ?id
           ?labels ?project ?timeouts ~forwarding_rules
           ~load_balancing_scheme ~location ~name ~extension_chains
           ());
    attrs = __attrs;
...
let register ?tf_module ?description ?id ?labels ?project ?timeouts
  ...
  let (r : _ Tf_core.resource) =
    make ?description ?id ?labels ?project ?timeouts
      ~forwarding_rules ~load_balancing_scheme ~location ~name
      ~extension_chains __id
  in
