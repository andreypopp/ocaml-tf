...
rest/v1/projects.locations.runtimes#RuntimeAccessType'. *)
  ...
  runtime_owner : string option; [@option]
...
If not specified, we'll automatically choose from official GPU drivers. *)
  enable_health_monitoring : bool option; [@option]
      (** Verifies core internal services are running. Default: True. *)
  idle_shutdown : bool option; [@option]
      (** Runtime will automatically shutdown after idle_shutdown_time.
Default: True *)
  idle_shutdown_timeout : float option; [@option]
      (** Time in minutes to wait before shuting down runtime.
Default: 180 minutes *)
  install_gpu_driver : bool option; [@option]
      (** Install Nvidia Driver automatically. *)
  notebook_upgrade_schedule : string option; [@option]
...
Cloud Storage path (gs://path-to-file/file-name). *)
  post_startup_script_behavior : string option; [@option]
...
type google_notebooks_runtime__virtual_machine__virtual_machine_config__accelerator_config = {
  core_count : float option; [@option]
...
type google_notebooks_runtime__virtual_machine__virtual_machine_config = {
  guest_attributes : (string * string) list;
