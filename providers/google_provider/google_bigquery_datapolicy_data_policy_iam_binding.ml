...
let google_bigquery_datapolicy_data_policy_iam_binding
    ~data_policy_id ~members ~role ~condition __resource_id =
  let __resource_type =
    "google_bigquery_datapolicy_data_policy_iam_binding"
  in
  let __resource = { data_policy_id; members; role; condition } in
