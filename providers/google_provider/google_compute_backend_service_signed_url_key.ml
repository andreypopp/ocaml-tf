...
let google_compute_backend_service_signed_url_key ?timeouts
    ~backend_service ~key_value ~name __resource_id =
  let __resource_type =
    "google_compute_backend_service_signed_url_key"
  in
  let __resource = { backend_service; key_value; name; timeouts } in
