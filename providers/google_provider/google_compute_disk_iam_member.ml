...
let google_compute_disk_iam_member ~member ~name ~role ~condition
    __resource_id =
  let __resource_type = "google_compute_disk_iam_member" in
  let __resource = { member; name; role; condition } in
  Resource.add ~type_:__resource_type ~id:__resource_id
    (yojson_of_google_compute_disk_iam_member __resource);
  ()
