...
type google_network_services_edge_cache_service__routing__path_matcher__route_rule__route_action__cdn_policy__add_signatures = {
  ...
  copied_parameters : string list option; [@option]
...
This must be specified when the GENERATE_COOKIE or GENERATE_TOKEN_HLS_COOKIELESS actions are specified.  This field may not be specified otherwise. *)
  token_query_parameter : string option; [@option]
...
type google_network_services_edge_cache_service__routing__path_matcher__route_rule__route_action__cdn_policy__cache_key_policy = {
  exclude_host : bool option; [@option]
      (** If true, requests to different hosts will be cached separately.

Note: this should only be enabled if hosts share the same origin and content. Removing the host from the cache key may inadvertently result in different objects being cached than intended, depending on which route the first user matched. *)
  exclude_query_string : bool option; [@option]
...
included. *)
  excluded_query_parameters : string list option; [@option]
      (** Names of query string parameters to exclude from cache keys. All other parameters will be included.

Either specify includedQueryParameters or excludedQueryParameters, not both. '&' and '=' will be percent encoded and not treated as delimiters. *)
  include_protocol : bool option; [@option]
      (** If true, http and https requests will be cached separately. *)
  included_cookie_names : string list option; [@option]
...
Cookie names:
  ...
  included_header_names : string list option; [@option]
...
Note that specifying several headers, and/or headers that have a large range of values (e.g. per-user) will dramatically impact the cache hit rate, and may result in a higher eviction rate and reduced performance. *)
  included_query_parameters : string list option; [@option]
...
type google_network_services_edge_cache_service__routing__path_matcher__route_rule__route_action__cdn_policy__signed_token_options = {
  allowed_signature_algorithms : string list option; [@option]
...
For all cache modes, Cache-Control headers will be passed to the client. Use clientTtl to override what is sent to the client. Possible values: [CACHE_ALL_STATIC, USE_ORIGIN_HEADERS, FORCE_CACHE_ALL, BYPASS_CACHE] *)
  client_ttl : string option; [@option]
...
A duration in seconds terminated by 's'. Example: 3s. *)
  default_ttl : string option; [@option]
...
These defaults can be overridden in negativeCachingPolicy *)
  negative_caching_policy : (string * string) list option; [@option]
...
Note that when specifying an explicit negativeCachingPolicy, you should take care to specify a cache TTL for all response codes that you wish to cache. The CDNPolicy will not apply any default negative caching when a policy exists. *)
  signed_request_keyset : string option; [@option]
      (** The EdgeCacheKeyset containing the set of public keys used to validate signed requests at the edge. *)
  signed_request_maximum_expiration_ttl : string option; [@option]
...
By default, signedRequestMaximumExpirationTtl is not set and the expiration time of a signed request may be arbitrarily far into future. *)
  signed_request_mode : string option; [@option]
...
type google_network_services_edge_cache_service = {
  description : string option; [@option]
      (** A human-readable description of the resource. *)
  disable_http2 : bool option; [@option]
      (** Disables HTTP/2.
