...
let google_bigquery_analytics_hub_listing_iam_policy
    ~data_exchange_id ~listing_id ~policy_data __resource_id =
  let __resource_type =
    "google_bigquery_analytics_hub_listing_iam_policy"
  in
  let __resource = { data_exchange_id; listing_id; policy_data } in
