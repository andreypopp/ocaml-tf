...
let _ = fun (_ : azurerm_eventhub_authorization_rule) -> ()

let yojson_of_azurerm_eventhub_authorization_rule =
  (function
   | {
...
let _ = yojson_of_azurerm_eventhub_authorization_rule
...
let make ?id ?listen ?manage ?send ?timeouts ~eventhub_name ~name
    ~namespace_name ~resource_group_name __id =
  let __type = "azurerm_eventhub_authorization_rule" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_eventhub_authorization_rule
        (azurerm_eventhub_authorization_rule ?id ?listen ?manage
           ?send ?timeouts ~eventhub_name ~name ~namespace_name
           ~resource_group_name ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?listen ?manage ?send ?timeouts
    ~eventhub_name ~name ~namespace_name ~resource_group_name __id =
  let (r : _ Tf_core.resource) =
    make ?id ?listen ?manage ?send ?timeouts ~eventhub_name ~name
      ~namespace_name ~resource_group_name __id
  in
