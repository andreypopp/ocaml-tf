...
let _ = fun (_ : azurerm_mariadb_database) -> ()

let yojson_of_azurerm_mariadb_database =
  (function
   | {
       charset = v_charset;
       ...
       in
       `Assoc bnds
    : azurerm_mariadb_database -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_azurerm_mariadb_database
...
let make ?id ?timeouts ~charset ~collation ~name ~resource_group_name
    ~server_name __id =
  let __type = "azurerm_mariadb_database" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_mariadb_database
        (azurerm_mariadb_database ?id ?timeouts ~charset ~collation
