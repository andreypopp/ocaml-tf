...
let _ = fun (_ : azurerm_web_app_active_slot) -> ()

let yojson_of_azurerm_web_app_active_slot =
  (function
   | {
       id = v_id;
...
let _ = yojson_of_azurerm_web_app_active_slot
...
let make ?id ?overwrite_network_config ?timeouts ~slot_id __id =
  let __type = "azurerm_web_app_active_slot" in
  let __attrs =
    ({
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_web_app_active_slot
