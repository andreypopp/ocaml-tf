...
let _ = fun (_ : azurerm_spring_cloud_accelerator) -> ()

let yojson_of_azurerm_spring_cloud_accelerator =
  (function
   | {
       id = v_id;
       name = v_name;
...
let _ = yojson_of_azurerm_spring_cloud_accelerator
...
let make ?id ?timeouts ~name ~spring_cloud_service_id __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_spring_cloud_accelerator
