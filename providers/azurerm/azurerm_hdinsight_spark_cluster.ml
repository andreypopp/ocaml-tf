...
type component_version = { spark : string prop }
...
let yojson_of_component_version =
  (function
   | { spark = v_spark } ->
       ...
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_spark in
         ("spark", arg) :: bnds
       in
       `Assoc bnds
...
let _ = fun (_ : azurerm_hdinsight_spark_cluster) -> ()

let yojson_of_azurerm_hdinsight_spark_cluster =
  (function
   | {
...
let _ = yojson_of_azurerm_hdinsight_spark_cluster

[@@@deriving.end]

let component_version ~spark () : component_version = { spark }
...
let register ?tf_module ?encryption_in_transit_enabled ?id ?tags
  ...
  let (r : _ Tf_core.resource) =
    make ?encryption_in_transit_enabled ?id ?tags ?tls_min_version
      ~compute_isolation ~disk_encryption ~extension ~metastores
      ~monitor ~network ~private_link_configuration ~security_profile
      ~storage_account ~storage_account_gen2 ?timeouts
      ~cluster_version ~location ~name ~resource_group_name ~tier
      ~component_version ~gateway ~roles __id
  in
