...
let _ = fun (_ : azurerm_postgresql_server_key) -> ()

let yojson_of_azurerm_postgresql_server_key =
  (function
   | {
       id = v_id;
...
let _ = yojson_of_azurerm_postgresql_server_key
...
let make ?id ?timeouts ~key_vault_key_id ~server_id __id =
  let __type = "azurerm_postgresql_server_key" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_postgresql_server_key
        (azurerm_postgresql_server_key ?id ?timeouts
           ~key_vault_key_id ~server_id ());
    attrs = __attrs;
  }
