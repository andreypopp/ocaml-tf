  ...
  (function
   | {
       ...
       let bnds =
         ...
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_list yojson_of_rule v in
             let bnd = "rule", arg in
             bnd :: bnds
       in
       let bnds =
...
let azurerm_route_filter ?id ?rule ?tags ?timeouts ~location ~name
    ~resource_group_name () : azurerm_route_filter =
...
let make ?id ?rule ?tags ?timeouts ~location ~name
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_route_filter
        (azurerm_route_filter ?id ?rule ?tags ?timeouts ~location
           ~name ~resource_group_name ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?rule ?tags ?timeouts ~location ~name
    ~resource_group_name __id =
  let (r : _ Tf_core.resource) =
    make ?id ?rule ?tags ?timeouts ~location ~name
      ~resource_group_name __id
  in
