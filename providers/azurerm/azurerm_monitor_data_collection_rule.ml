...
let yojson_of_azurerm_monitor_data_collection_rule =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_stream_declaration then bnds
         else
           let arg =
             (yojson_of_list yojson_of_stream_declaration)
               v_stream_declaration
           in
           let bnd = "stream_declaration", arg in
           bnd :: bnds
       in
       let bnds =
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_data_sources then bnds
         else
           let arg =
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_data_flow then bnds
         else
           let arg =
             (yojson_of_list yojson_of_data_flow) v_data_flow
           in
           let bnd = "data_flow", arg in
           bnd :: bnds
       in
       let bnds =
         match v_tags with
