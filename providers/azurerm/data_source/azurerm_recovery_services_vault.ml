...
let azurerm_recovery_services_vault ?id ?timeouts ~name
    ~resource_group_name () : azurerm_recovery_services_vault =
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_recovery_services_vault
        (azurerm_recovery_services_vault ?id ?timeouts ~name
           ~resource_group_name ());
    attrs = __attrs;
  }
