...
type azurerm_sentinel_alert_rule_anomaly = {
...
let _ = fun (_ : azurerm_sentinel_alert_rule_anomaly) -> ()

let yojson_of_azurerm_sentinel_alert_rule_anomaly =
  (function
   | {
...
let _ = yojson_of_azurerm_sentinel_alert_rule_anomaly

[@@@deriving.end]

let timeouts ?read () : timeouts = { read }

let azurerm_sentinel_alert_rule_anomaly ?display_name ?id ?name
...
let make ?display_name ?id ?name ?timeouts
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_sentinel_alert_rule_anomaly
        (azurerm_sentinel_alert_rule_anomaly ?display_name ?id ?name
           ?timeouts ~log_analytics_workspace_id ());
    attrs = __attrs;
  }
