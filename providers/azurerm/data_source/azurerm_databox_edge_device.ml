...
let yojson_of_device_properties =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_configured_role_types then bnds
         else
           let arg =
...
let azurerm_databox_edge_device ?id ?timeouts ~name
    ~resource_group_name () : azurerm_databox_edge_device =
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_databox_edge_device
        (azurerm_databox_edge_device ?id ?timeouts ~name
           ~resource_group_name ());
    attrs = __attrs;
  }
