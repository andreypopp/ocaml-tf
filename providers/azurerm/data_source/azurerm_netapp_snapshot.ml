...
let azurerm_netapp_snapshot ?id ?timeouts ~account_name ~name
    ~pool_name ~resource_group_name ~volume_name () :
...
let make ?id ?timeouts ~account_name ~name ~pool_name
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_netapp_snapshot
        (azurerm_netapp_snapshot ?id ?timeouts ~account_name ~name
           ~pool_name ~resource_group_name ~volume_name ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?timeouts ~account_name ~name ~pool_name
    ~resource_group_name ~volume_name __id =
  let (r : _ Tf_core.resource) =
    make ?id ?timeouts ~account_name ~name ~pool_name
      ~resource_group_name ~volume_name __id
  in
