...
let azurerm_servicebus_namespace_authorization_rule ?id ?namespace_id
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_servicebus_namespace_authorization_rule
        (azurerm_servicebus_namespace_authorization_rule ?id
           ?namespace_id ?namespace_name ?resource_group_name
           ?timeouts ~name ());
    attrs = __attrs;
  }
