...
let _ = fun (_ : azurerm_servicebus_queue_authorization_rule) -> ()

let yojson_of_azurerm_servicebus_queue_authorization_rule =
  (function
   | {
       id = v_id;
       name = v_name;
...
let _ = yojson_of_azurerm_servicebus_queue_authorization_rule

[@@@deriving.end]

let timeouts ?read () : timeouts = { read }

let azurerm_servicebus_queue_authorization_rule ?id ?namespace_name
    ?queue_id ?queue_name ?resource_group_name ?timeouts ~name () :
    azurerm_servicebus_queue_authorization_rule =
  {
    id;
    name;
...
let make ?id ?namespace_name ?queue_id ?queue_name
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_servicebus_queue_authorization_rule
        (azurerm_servicebus_queue_authorization_rule ?id
           ?namespace_name ?queue_id ?queue_name ?resource_group_name
           ?timeouts ~name ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?namespace_name ?queue_id ?queue_name
    ?resource_group_name ?timeouts ~name __id =
  let (r : _ Tf_core.resource) =
    make ?id ?namespace_name ?queue_id ?queue_name
      ?resource_group_name ?timeouts ~name __id
  in
