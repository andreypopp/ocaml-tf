...
let azurerm_container_group ?id ?zones ?timeouts ~name
    ~resource_group_name () : azurerm_container_group =
...
let make ?id ?zones ?timeouts ~name ~resource_group_name __id =
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_container_group
        (azurerm_container_group ?id ?zones ?timeouts ~name
           ~resource_group_name ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?zones ?timeouts ~name
    ~resource_group_name __id =
  let (r : _ Tf_core.resource) =
    make ?id ?zones ?timeouts ~name ~resource_group_name __id
  in
