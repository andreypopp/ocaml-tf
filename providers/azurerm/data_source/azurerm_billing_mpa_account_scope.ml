...
let _ = fun (_ : azurerm_billing_mpa_account_scope) -> ()

let yojson_of_azurerm_billing_mpa_account_scope =
  (function
   | {
...
let _ = yojson_of_azurerm_billing_mpa_account_scope

[@@@deriving.end]

let timeouts ?read () : timeouts = { read }

let azurerm_billing_mpa_account_scope ?id ?timeouts
    ~billing_account_name ~customer_name () :
    azurerm_billing_mpa_account_scope =
...
let make ?id ?timeouts ~billing_account_name ~customer_name __id =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_billing_mpa_account_scope
        (azurerm_billing_mpa_account_scope ?id ?timeouts
           ~billing_account_name ~customer_name ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?timeouts ~billing_account_name
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?timeouts ~billing_account_name ~customer_name __id
  in
