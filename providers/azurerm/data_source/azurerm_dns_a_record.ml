...
let _ = fun (_ : azurerm_dns_a_record) -> ()

let yojson_of_azurerm_dns_a_record =
  (function
   | {
       id = v_id;
       name = v_name;
       ...
       in
       `Assoc bnds
    : azurerm_dns_a_record -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_azurerm_dns_a_record

[@@@deriving.end]

let timeouts ?read () : timeouts = { read }

let azurerm_dns_a_record ?id ?timeouts ~name ~resource_group_name
    ~zone_name () : azurerm_dns_a_record =
...
let make ?id ?timeouts ~name ~resource_group_name ~zone_name __id =
  let __type = "azurerm_dns_a_record" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_dns_a_record
        (azurerm_dns_a_record ?id ?timeouts ~name
