...
type autoscale_settings = { max_throughput : float prop }
[@@deriving_inline yojson_of]
...
let yojson_of_autoscale_settings =
  (function
   | { max_throughput = v_max_throughput } ->
       ...
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_float v_max_throughput in
         ("max_throughput", arg) :: bnds
       in
       `Assoc bnds
...
let _ = fun (_ : azurerm_cosmosdb_sql_database) -> ()

let yojson_of_azurerm_cosmosdb_sql_database =
  (function
   | {
...
let _ = yojson_of_azurerm_cosmosdb_sql_database

[@@@deriving.end]

let timeouts ?read () : timeouts = { read }

let azurerm_cosmosdb_sql_database ?id ?timeouts ~account_name ~name
    ~resource_group_name () : azurerm_cosmosdb_sql_database =
...
let make ?id ?timeouts ~account_name ~name ~resource_group_name __id
    =
  let __type = "azurerm_cosmosdb_sql_database" in
  let __attrs =
    ({
       account_name = Prop.computed __type __id "account_name";
       autoscale_settings =
         Prop.computed __type __id "autoscale_settings";
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_cosmosdb_sql_database
        (azurerm_cosmosdb_sql_database ?id ?timeouts ~account_name
