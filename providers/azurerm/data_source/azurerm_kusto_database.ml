...
let azurerm_kusto_database ?id ?timeouts ~cluster_name ~name
    ~resource_group_name () : azurerm_kusto_database =
...
let make ?id ?timeouts ~cluster_name ~name ~resource_group_name __id
    =
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_kusto_database
        (azurerm_kusto_database ?id ?timeouts ~cluster_name ~name
           ~resource_group_name ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?timeouts ~cluster_name ~name
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?timeouts ~cluster_name ~name ~resource_group_name __id
  in
