...
let _ = fun (_ : azurerm_ssh_public_key) -> ()

let yojson_of_azurerm_ssh_public_key =
  (function
   | {
       id = v_id;
       name = v_name;
       ...
       in
       `Assoc bnds
    : azurerm_ssh_public_key -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_azurerm_ssh_public_key

[@@@deriving.end]

let timeouts ?read () : timeouts = { read }

let azurerm_ssh_public_key ?id ?tags ?timeouts ~name
    ~resource_group_name () : azurerm_ssh_public_key =
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_ssh_public_key
        (azurerm_ssh_public_key ?id ?tags ?timeouts ~name
           ~resource_group_name ());
    attrs = __attrs;
  }
