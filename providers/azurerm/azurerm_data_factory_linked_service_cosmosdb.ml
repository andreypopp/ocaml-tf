...
let _ = fun (_ : azurerm_data_factory_linked_service_cosmosdb) -> ()

let yojson_of_azurerm_data_factory_linked_service_cosmosdb =
  (function
   | {
       account_endpoint = v_account_endpoint;
       ...
       in
       let bnds =
         match v_account_endpoint with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "account_endpoint", arg in
             bnd :: bnds
       in
       `Assoc bnds
...
let _ = yojson_of_azurerm_data_factory_linked_service_cosmosdb
...
let make ?account_endpoint ?account_key ?additional_properties
    ?annotations ?connection_string ?database ?description ?id
    ?integration_runtime_name ?parameters ?timeouts ~data_factory_id
    ~name __id =
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_data_factory_linked_service_cosmosdb
        (azurerm_data_factory_linked_service_cosmosdb
           ?account_endpoint ?account_key ?additional_properties
           ?annotations ?connection_string ?database ?description ?id
...
let register ?tf_module ?account_endpoint ?account_key
  ...
  let (r : _ Tf_core.resource) =
    make ?account_endpoint ?account_key ?additional_properties
      ?annotations ?connection_string ?database ?description ?id
      ?integration_runtime_name ?parameters ?timeouts
      ~data_factory_id ~name __id
  in
