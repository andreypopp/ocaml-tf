...
let _ = fun (_ : azurerm_spring_cloud_gateway_custom_domain) -> ()

let yojson_of_azurerm_spring_cloud_gateway_custom_domain =
  (function
   | {
       id = v_id;
       name = v_name;
...
let _ = yojson_of_azurerm_spring_cloud_gateway_custom_domain
...
let azurerm_spring_cloud_gateway_custom_domain ?id ?thumbprint
    ?timeouts ~name ~spring_cloud_gateway_id () :
    azurerm_spring_cloud_gateway_custom_domain =
...
let make ?id ?thumbprint ?timeouts ~name ~spring_cloud_gateway_id
    __id =
  let __type = "azurerm_spring_cloud_gateway_custom_domain" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_spring_cloud_gateway_custom_domain
        (azurerm_spring_cloud_gateway_custom_domain ?id ?thumbprint
           ?timeouts ~name ~spring_cloud_gateway_id ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?thumbprint ?timeouts ~name
    ~spring_cloud_gateway_id __id =
  let (r : _ Tf_core.resource) =
    make ?id ?thumbprint ?timeouts ~name ~spring_cloud_gateway_id
      __id
  in
