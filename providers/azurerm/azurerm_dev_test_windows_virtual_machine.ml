...
let _ = fun (_ : azurerm_dev_test_windows_virtual_machine) -> ()

let yojson_of_azurerm_dev_test_windows_virtual_machine =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_inbound_nat_rule then bnds
         else
           let arg =
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_gallery_image_reference then bnds
         else
           let arg =
...
let _ = yojson_of_azurerm_dev_test_windows_virtual_machine
