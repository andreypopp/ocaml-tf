...
let _ = fun (_ : azurerm_resource_policy_assignment) -> ()

let yojson_of_azurerm_resource_policy_assignment =
  (function
   | {
...
let _ = yojson_of_azurerm_resource_policy_assignment
...
let azurerm_resource_policy_assignment ?description ?display_name
    ...
    ~resource_id () : azurerm_resource_policy_assignment =
  {
    description;
    display_name;
    enforce;
...
let register ?tf_module ?description ?display_name ?enforce ?id
  ...
  let (r : _ Tf_core.resource) =
    make ?description ?display_name ?enforce ?id ?location ?metadata
      ...
      ~policy_definition_id ~resource_id __id
  in
