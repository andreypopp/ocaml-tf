...
let _ = fun (_ : azurerm_resource_policy_exemption) -> ()

let yojson_of_azurerm_resource_policy_exemption =
  (function
   | {
...
let _ = yojson_of_azurerm_resource_policy_exemption
...
let azurerm_resource_policy_exemption ?description ?display_name
    ...
    ~resource_id () : azurerm_resource_policy_exemption =
  {
    description;
    display_name;
    exemption_category;
...
let make ?description ?display_name ?expires_on ?id ?metadata
    ...
    ~name ~policy_assignment_id ~resource_id __id =
  let __type = "azurerm_resource_policy_exemption" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_resource_policy_exemption
        (azurerm_resource_policy_exemption ?description ?display_name
           ?expires_on ?id ?metadata ?policy_definition_reference_ids
           ?timeouts ~exemption_category ~name ~policy_assignment_id
           ~resource_id ());
    attrs = __attrs;
...
let register ?tf_module ?description ?display_name ?expires_on ?id
    ...
    ~exemption_category ~name ~policy_assignment_id ~resource_id __id
    =
