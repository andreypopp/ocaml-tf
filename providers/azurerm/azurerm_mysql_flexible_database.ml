...
let _ = fun (_ : azurerm_mysql_flexible_database) -> ()

let yojson_of_azurerm_mysql_flexible_database =
  (function
   | {
       charset = v_charset;
...
let _ = yojson_of_azurerm_mysql_flexible_database
...
let make ?id ?timeouts ~charset ~collation ~name ~resource_group_name
    ~server_name __id =
  let __type = "azurerm_mysql_flexible_database" in
  let __attrs =
    ({
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_mysql_flexible_database
        (azurerm_mysql_flexible_database ?id ?timeouts ~charset
