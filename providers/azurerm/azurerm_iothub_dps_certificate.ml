...
let _ = fun (_ : azurerm_iothub_dps_certificate) -> ()

let yojson_of_azurerm_iothub_dps_certificate =
  (function
   | {
       certificate_content = v_certificate_content;
       id = v_id;
       iot_dps_name = v_iot_dps_name;
       ...
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_iot_dps_name in
         ("iot_dps_name", arg) :: bnds
       in
       let bnds =
         match v_id with
         | Ppx_yojson_conv_lib.Option.None -> bnds
...
let _ = yojson_of_azurerm_iothub_dps_certificate
