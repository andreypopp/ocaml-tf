...
let make ?id ?listen ?manage ?send ?timeouts ~name ~topic_id __id =
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_servicebus_topic_authorization_rule
        (azurerm_servicebus_topic_authorization_rule ?id ?listen
           ?manage ?send ?timeouts ~name ~topic_id ());
    attrs = __attrs;
  }
...
let register ?tf_module ?id ?listen ?manage ?send ?timeouts ~name
  ...
  let (r : _ Tf_core.resource) =
    make ?id ?listen ?manage ?send ?timeouts ~name ~topic_id __id
  in
