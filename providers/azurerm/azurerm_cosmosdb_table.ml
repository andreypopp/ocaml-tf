...
let _ = fun (_ : azurerm_cosmosdb_table) -> ()

let yojson_of_azurerm_cosmosdb_table =
  (function
   | {
       ...
       in
       `Assoc bnds
    : azurerm_cosmosdb_table -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_azurerm_cosmosdb_table
...
let azurerm_cosmosdb_table ?id ?throughput ?(autoscale_settings = [])
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_cosmosdb_table
        (azurerm_cosmosdb_table ?id ?throughput ~autoscale_settings
           ?timeouts ~account_name ~name ~resource_group_name ());
    attrs = __attrs;
  }
