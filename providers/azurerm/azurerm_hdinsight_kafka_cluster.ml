...
type component_version = { kafka : string prop }
...
let yojson_of_component_version =
  (function
   | { kafka = v_kafka } ->
       ...
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_kafka in
         ("kafka", arg) :: bnds
       in
       `Assoc bnds
...
let _ = fun (_ : rest_proxy) -> ()

let yojson_of_rest_proxy =
  (function
   | {
       security_group_id = v_security_group_id;
       security_group_name = v_security_group_name;
     } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       in
       let bnds =
         let arg =
           yojson_of_prop yojson_of_string v_security_group_name
         in
         ("security_group_name", arg) :: bnds
       in
       let bnds =
         let arg =
       ...
       in
       `Assoc bnds
    : rest_proxy -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_rest_proxy
...
let _ = fun (_ : roles__kafka_management_node__script_actions) -> ()

let yojson_of_roles__kafka_management_node__script_actions =
  (function
   | { name = v_name; parameters = v_parameters; uri = v_uri } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
...
let _ = yojson_of_roles__kafka_management_node__script_actions
...
let _ = fun (_ : roles__kafka_management_node) -> ()

let yojson_of_roles__kafka_management_node =
  (function
   | {
       password = v_password;
       ssh_keys = v_ssh_keys;
...
let _ = yojson_of_roles__kafka_management_node
...
let yojson_of_roles__worker_node =
  (function
   | {
       number_of_disks_per_node = v_number_of_disks_per_node;
       password = v_password;
       ssh_keys = v_ssh_keys;
       subnet_id = v_subnet_id;
       ...
       let bnds =
         let arg =
           yojson_of_prop yojson_of_float v_number_of_disks_per_node
         in
         ("number_of_disks_per_node", arg) :: bnds
       in
       `Assoc bnds
...
let yojson_of_roles =
  (function
   | {
       ...
       kafka_management_node = v_kafka_management_node;
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_kafka_management_node then bnds
         else
           let arg =
...
let _ = fun (_ : azurerm_hdinsight_kafka_cluster) -> ()

let yojson_of_azurerm_hdinsight_kafka_cluster =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_rest_proxy then bnds
         else
           let arg =
...
let _ = yojson_of_azurerm_hdinsight_kafka_cluster

[@@@deriving.end]

let component_version ~kafka () : component_version = { kafka }
...
let rest_proxy ~security_group_id ~security_group_name () :
    rest_proxy =
...
let roles__kafka_management_node__script_actions ?parameters ~name
    ~uri () : roles__kafka_management_node__script_actions =
  { name; parameters; uri }
...
let azurerm_hdinsight_kafka_cluster ?encryption_in_transit_enabled
    ...
    ?(monitor = []) ?(network = [])
    ?(private_link_configuration = []) ?(rest_proxy = [])
    ...
    json =
      yojson_of_azurerm_hdinsight_kafka_cluster
        (azurerm_hdinsight_kafka_cluster
           ...
           ~monitor ~network ~private_link_configuration ~rest_proxy
