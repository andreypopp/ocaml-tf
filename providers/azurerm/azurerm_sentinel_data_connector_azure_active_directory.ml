...
let _ =
 fun (_ : azurerm_sentinel_data_connector_azure_active_directory) ->
  ()

let yojson_of_azurerm_sentinel_data_connector_azure_active_directory
    =
  (function
   | {
       id = v_id;
...
let make ?id ?tenant_id ?timeouts ~log_analytics_workspace_id ~name
  ...
  in
  {
    ...
    json =
      yojson_of_azurerm_sentinel_data_connector_azure_active_directory
        (azurerm_sentinel_data_connector_azure_active_directory ?id
           ?tenant_id ?timeouts ~log_analytics_workspace_id ~name ());
    attrs = __attrs;
  }
