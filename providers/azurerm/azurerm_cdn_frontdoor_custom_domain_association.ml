...
let _ =
 fun (_ : azurerm_cdn_frontdoor_custom_domain_association) -> ()

let yojson_of_azurerm_cdn_frontdoor_custom_domain_association =
  (function
   | {
       cdn_frontdoor_custom_domain_id =
         v_cdn_frontdoor_custom_domain_id;
       cdn_frontdoor_route_ids = v_cdn_frontdoor_route_ids;
       id = v_id;
       timeouts = v_timeouts;
     } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         ...
         if [] = v_cdn_frontdoor_route_ids then bnds
           ...
           let arg =
             (yojson_of_list (yojson_of_prop yojson_of_string))
               v_cdn_frontdoor_route_ids
           in
           let bnd = "cdn_frontdoor_route_ids", arg in
           bnd :: bnds
       in
       let bnds =
         let arg =
           yojson_of_prop yojson_of_string
             v_cdn_frontdoor_custom_domain_id
         in
         ("cdn_frontdoor_custom_domain_id", arg) :: bnds
       in
       `Assoc bnds
    : azurerm_cdn_frontdoor_custom_domain_association ->
      Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_azurerm_cdn_frontdoor_custom_domain_association
