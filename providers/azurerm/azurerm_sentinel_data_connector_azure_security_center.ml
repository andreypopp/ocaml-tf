...
let _ =
 fun (_ : azurerm_sentinel_data_connector_azure_security_center) ->
  ()

let yojson_of_azurerm_sentinel_data_connector_azure_security_center =
  (function
   | {
       id = v_id;
    ...
    json =
      yojson_of_azurerm_sentinel_data_connector_azure_security_center
        (azurerm_sentinel_data_connector_azure_security_center ?id
           ?subscription_id ?timeouts ~log_analytics_workspace_id
           ~name ());
    attrs = __attrs;
  }
