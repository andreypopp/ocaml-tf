...
let _ = fun (_ : azurerm_mysql_virtual_network_rule) -> ()

let yojson_of_azurerm_mysql_virtual_network_rule =
  (function
   | {
       id = v_id;
       name = v_name;
...
let _ = yojson_of_azurerm_mysql_virtual_network_rule
...
let make ?id ?timeouts ~name ~resource_group_name ~server_name
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_mysql_virtual_network_rule
        (azurerm_mysql_virtual_network_rule ?id ?timeouts ~name
           ~resource_group_name ~server_name ~subnet_id ());
    attrs = __attrs;
  }
