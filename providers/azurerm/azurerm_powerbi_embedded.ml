...
let _ = fun (_ : azurerm_powerbi_embedded) -> ()

let yojson_of_azurerm_powerbi_embedded =
  (function
   | {
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_administrators then bnds
         else
           let arg =
...
let _ = yojson_of_azurerm_powerbi_embedded
...
let make ?id ?mode ?tags ?timeouts ~administrators ~location ~name
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_powerbi_embedded
        (azurerm_powerbi_embedded ?id ?mode ?tags ?timeouts
           ~administrators ~location ~name ~resource_group_name
           ~sku_name ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?mode ?tags ?timeouts ~administrators
    ~location ~name ~resource_group_name ~sku_name __id =
  let (r : _ Tf_core.resource) =
    make ?id ?mode ?tags ?timeouts ~administrators ~location ~name
      ~resource_group_name ~sku_name __id
  in
