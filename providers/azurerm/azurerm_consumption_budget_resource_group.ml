...
let azurerm_consumption_budget_resource_group ?etag ?id ?time_grain
    ?(filter = []) ?timeouts ~amount ~name ~resource_group_id
...
let make ?etag ?id ?time_grain ?(filter = []) ?timeouts ~amount ~name
    ~resource_group_id ~notification ~time_period __id =
    ...
    json =
      yojson_of_azurerm_consumption_budget_resource_group
        (azurerm_consumption_budget_resource_group ?etag ?id
           ...
           ~resource_group_id ~notification ~time_period ());
    attrs = __attrs;
  }

let register ?tf_module ?etag ?id ?time_grain ?(filter = [])
    ?timeouts ~amount ~name ~resource_group_id ~notification
    ~time_period __id =
