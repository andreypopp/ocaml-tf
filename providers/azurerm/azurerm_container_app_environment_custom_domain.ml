...
let _ =
 fun (_ : azurerm_container_app_environment_custom_domain) -> ()

let yojson_of_azurerm_container_app_environment_custom_domain =
  (function
   | {
...
let _ = yojson_of_azurerm_container_app_environment_custom_domain
...
let azurerm_container_app_environment_custom_domain ?id ?timeouts
...
let make ?id ?timeouts ~certificate_blob_base64 ~certificate_password
    ~container_app_environment_id ~dns_suffix __id =
  let __type = "azurerm_container_app_environment_custom_domain" in
  let __attrs =
    ({
       tf_name = __id;
       certificate_blob_base64 =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_container_app_environment_custom_domain
        (azurerm_container_app_environment_custom_domain ?id
           ?timeouts ~certificate_blob_base64 ~certificate_password
           ~container_app_environment_id ~dns_suffix ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?timeouts ~certificate_blob_base64
    ~certificate_password ~container_app_environment_id ~dns_suffix
    __id =
  let (r : _ Tf_core.resource) =
    make ?id ?timeouts ~certificate_blob_base64 ~certificate_password
      ~container_app_environment_id ~dns_suffix __id
  in
