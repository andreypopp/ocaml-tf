...
let yojson_of_blob_storage =
  (function
   | {
       ...
       let bnds =
         let arg =
           yojson_of_list yojson_of_blob_storage__service_principal
             v_service_principal
         in
...
let _ = fun (_ : azurerm_arc_kubernetes_flux_configuration) -> ()

let yojson_of_azurerm_arc_kubernetes_flux_configuration =
  (function
   | {
       ...
       let bnds =
         let arg =
           yojson_of_list yojson_of_kustomizations v_kustomizations
         in
         ("kustomizations", arg) :: bnds
       in
       let bnds =
         let arg =
...
let _ = yojson_of_azurerm_arc_kubernetes_flux_configuration
...
let blob_storage ?account_key ?local_auth_reference ?sas_token
    ...
    ?(service_principal = []) ~container_id () : blob_storage =
  {
    account_key;
    container_id;
...
let make ?continuous_reconciliation_enabled ?id ?scope
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_arc_kubernetes_flux_configuration
        (azurerm_arc_kubernetes_flux_configuration
           ?continuous_reconciliation_enabled ?id ?scope
