...
let _ = fun (_ : azurerm_linux_function_app_slot) -> ()

let yojson_of_azurerm_linux_function_app_slot =
  (function
   | {
...
let _ = yojson_of_azurerm_linux_function_app_slot
...
let site_config__application_stack__docker ?registry_password
    ?registry_username ~image_name ~image_tag ~registry_url () :
    site_config__application_stack__docker =
  {
    image_name;
    image_tag;
...
let site_config ?always_on ?api_definition_url ?api_management_api_id
    ...
    ?elastic_instance_minimum ?ftps_state
...
let azurerm_linux_function_app_slot ?app_settings
    ...
    ?(identity = []) ?timeouts ~function_app_id ~name
...
let make ?app_settings ?builtin_logging_enabled
  ...
  let __type = "azurerm_linux_function_app_slot" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_linux_function_app_slot
        (azurerm_linux_function_app_slot ?app_settings
           ...
           ?virtual_network_subnet_id
           ?webdeploy_publish_basic_authentication_enabled
           ~auth_settings ~auth_settings_v2 ~backup ~identity
           ?timeouts ~function_app_id ~name ~connection_string
...
let register ?tf_module ?app_settings ?builtin_logging_enabled
  ...
  let (r : _ Tf_core.resource) =
    make ?app_settings ?builtin_logging_enabled
      ...
      ?storage_account_name ?storage_key_vault_secret_id
      ?storage_uses_managed_identity ?tags ?virtual_network_subnet_id
      ?webdeploy_publish_basic_authentication_enabled ~auth_settings
      ~auth_settings_v2 ~backup ~identity ?timeouts ~function_app_id
      ~name ~connection_string ~site_config ~storage_account __id
  in
