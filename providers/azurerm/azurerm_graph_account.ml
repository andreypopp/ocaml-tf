...
let _ = fun (_ : azurerm_graph_account) -> ()

let yojson_of_azurerm_graph_account =
  (function
   | {
       ...
       in
       `Assoc bnds
    : azurerm_graph_account -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_azurerm_graph_account
...
let azurerm_graph_account ?id ?tags ?timeouts ~application_id ~name
    ~resource_group_name () : azurerm_graph_account =
...
let make ?id ?tags ?timeouts ~application_id ~name
  ...
  let __type = "azurerm_graph_account" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_azurerm_graph_account
        (azurerm_graph_account ?id ?tags ?timeouts ~application_id
