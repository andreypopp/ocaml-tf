...
type okta_app_auto_login = {
  ...
  sign_on_redirect_url : string prop option; [@option]
  sign_on_url : string prop option; [@option]
...
let _ = fun (_ : okta_app_auto_login) -> ()

let yojson_of_okta_app_auto_login =
  (function
   | {
       ...
       sign_on_redirect_url = v_sign_on_redirect_url;
       sign_on_url = v_sign_on_url;
       status = v_status;
       ...
       in
       let bnds =
         match v_sign_on_url with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "sign_on_url", arg in
             bnd :: bnds
       in
       let bnds =
         match v_sign_on_redirect_url with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_string v in
             let bnd = "sign_on_redirect_url", arg in
             bnd :: bnds
       in
       let bnds =
       ...
       in
       `Assoc bnds
    : okta_app_auto_login -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_okta_app_auto_login
...
let okta_app_auto_login ?accessibility_error_redirect_url
    ?accessibility_login_redirect_url ?accessibility_self_service
    ?admin_note ?app_links_json ?app_settings_json
    ?auto_submit_toolbar ?credentials_scheme ?enduser_note ?hide_ios
    ?hide_web ?id ?logo ?preconfigured_app ?reveal_password
    ?shared_password ?shared_username ?sign_on_redirect_url
    ?sign_on_url ?status ?user_name_template
    ?user_name_template_push_status ?user_name_template_suffix
    ?user_name_template_type ?timeouts ~label () :
    okta_app_auto_login =
  {
    accessibility_error_redirect_url;
    accessibility_login_redirect_url;
...
let make ?accessibility_error_redirect_url
    ...
    ?sign_on_url ?status ?user_name_template
    ?user_name_template_push_status ?user_name_template_suffix
    ?user_name_template_type ?timeouts ~label __id =
  let __type = "okta_app_auto_login" in
  let __attrs =
    ({
       tf_name = __id;
       accessibility_error_redirect_url =
       ...
       sign_on_redirect_url =
         Prop.computed __type __id "sign_on_redirect_url";
       sign_on_url = Prop.computed __type __id "sign_on_url";
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_app_auto_login
        (okta_app_auto_login ?accessibility_error_redirect_url
           ?accessibility_login_redirect_url
           ?accessibility_self_service ?admin_note ?app_links_json
           ?app_settings_json ?auto_submit_toolbar
           ?credentials_scheme ?enduser_note ?hide_ios ?hide_web ?id
           ?logo ?preconfigured_app ?reveal_password ?shared_password
           ?shared_username ?sign_on_redirect_url ?sign_on_url
           ?status ?user_name_template
           ?user_name_template_push_status ?user_name_template_suffix
           ?user_name_template_type ?timeouts ~label ());
    attrs = __attrs;
  }
...
let register ?tf_module ?accessibility_error_redirect_url
  ...
  let (r : _ Tf_core.resource) =
    make ?accessibility_error_redirect_url
      ...
      ?hide_ios ?hide_web ?id ?logo ?preconfigured_app
      ?reveal_password ?shared_password ?shared_username
      ?sign_on_redirect_url ?sign_on_url ?status ?user_name_template
      ?user_name_template_push_status ?user_name_template_suffix
      ?user_name_template_type ?timeouts ~label __id
  in
