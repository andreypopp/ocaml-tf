...
let _ = fun (_ : okta_policy_mfa) -> ()

let yojson_of_okta_policy_mfa =
  (function
   | {
       ...
       in
       `Assoc bnds
    : okta_policy_mfa -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_okta_policy_mfa

[@@@deriving.end]

let okta_policy_mfa ?description ?duo ?external_idp ?fido_u2f
    ?fido_webauthn ?google_otp ?groups_included ?hotp ?id ?is_oie
    ?okta_call ?okta_email ?okta_otp ?okta_password ?okta_push
    ?okta_question ?okta_sms ?okta_verify ?onprem_mfa ?phone_number
    ?priority ?rsa_token ?security_question ?status ?symantec_vip
    ?webauthn ?yubikey_token ~name () : okta_policy_mfa =
  {
    description;
    duo;
    external_idp;
...
let make ?description ?duo ?external_idp ?fido_u2f ?fido_webauthn
    ?google_otp ?groups_included ?hotp ?id ?is_oie ?okta_call
    ?okta_email ?okta_otp ?okta_password ?okta_push ?okta_question
    ?okta_sms ?okta_verify ?onprem_mfa ?phone_number ?priority
    ?rsa_token ?security_question ?status ?symantec_vip ?webauthn
    ?yubikey_token ~name __id =
  let __type = "okta_policy_mfa" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_policy_mfa
        (okta_policy_mfa ?description ?duo ?external_idp ?fido_u2f
           ?fido_webauthn ?google_otp ?groups_included ?hotp ?id
           ?is_oie ?okta_call ?okta_email ?okta_otp ?okta_password
           ?okta_push ?okta_question ?okta_sms ?okta_verify
           ?onprem_mfa ?phone_number ?priority ?rsa_token
           ?security_question ?status ?symantec_vip ?webauthn
           ?yubikey_token ~name ());
    attrs = __attrs;
  }

let register ?tf_module ?description ?duo ?external_idp ?fido_u2f
    ?fido_webauthn ?google_otp ?groups_included ?hotp ?id ?is_oie
    ?okta_call ?okta_email ?okta_otp ?okta_password ?okta_push
    ?okta_question ?okta_sms ?okta_verify ?onprem_mfa ?phone_number
    ?priority ?rsa_token ?security_question ?status ?symantec_vip
    ?webauthn ?yubikey_token ~name __id =
  let (r : _ Tf_core.resource) =
    make ?description ?duo ?external_idp ?fido_u2f ?fido_webauthn
      ?google_otp ?groups_included ?hotp ?id ?is_oie ?okta_call
      ?okta_email ?okta_otp ?okta_password ?okta_push ?okta_question
      ?okta_sms ?okta_verify ?onprem_mfa ?phone_number ?priority
      ?rsa_token ?security_question ?status ?symantec_vip ?webauthn
