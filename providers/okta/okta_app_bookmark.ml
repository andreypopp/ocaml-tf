...
type okta_app_bookmark = {
  ...
  request_integration : bool prop option; [@option]
...
let _ = fun (_ : okta_app_bookmark) -> ()

let yojson_of_okta_app_bookmark =
  (function
   | {
       ...
       label = v_label;
       logo = v_logo;
       request_integration = v_request_integration;
       status = v_status;
       url = v_url;
       timeouts = v_timeouts;
     } ->
       ...
       in
       let bnds =
         match v_request_integration with
         | Ppx_yojson_conv_lib.Option.None -> bnds
         | Ppx_yojson_conv_lib.Option.Some v ->
             let arg = yojson_of_prop yojson_of_bool v in
             let bnd = "request_integration", arg in
             bnd :: bnds
       in
       let bnds =
         match v_logo with
       ...
       in
       `Assoc bnds
    : okta_app_bookmark -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_okta_app_bookmark
...
let okta_app_bookmark ?accessibility_error_redirect_url
    ?accessibility_login_redirect_url ?accessibility_self_service
    ?admin_note ?app_links_json ?authentication_policy
    ?auto_submit_toolbar ?enduser_note ?hide_ios ?hide_web ?id ?logo
    ?request_integration ?status ?timeouts ~label ~url () :
    okta_app_bookmark =
  {
    accessibility_error_redirect_url;
    accessibility_login_redirect_url;
...
let make ?accessibility_error_redirect_url
    ...
    ?admin_note ?app_links_json ?authentication_policy
    ?auto_submit_toolbar ?enduser_note ?hide_ios ?hide_web ?id ?logo
    ?request_integration ?status ?timeouts ~label ~url __id =
  let __type = "okta_app_bookmark" in
  let __attrs =
    ({
       tf_name = __id;
       accessibility_error_redirect_url =
       ...
       request_integration =
         Prop.computed __type __id "request_integration";
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_app_bookmark
        (okta_app_bookmark ?accessibility_error_redirect_url
           ?accessibility_login_redirect_url
           ?accessibility_self_service ?admin_note ?app_links_json
           ?authentication_policy ?auto_submit_toolbar ?enduser_note
           ?hide_ios ?hide_web ?id ?logo ?request_integration ?status
           ?timeouts ~label ~url ());
    attrs = __attrs;
  }
...
let register ?tf_module ?accessibility_error_redirect_url
  ...
  let (r : _ Tf_core.resource) =
    make ?accessibility_error_redirect_url
      ?accessibility_login_redirect_url ?accessibility_self_service
      ?admin_note ?app_links_json ?authentication_policy
      ?auto_submit_toolbar ?enduser_note ?hide_ios ?hide_web ?id
      ?logo ?request_integration ?status ?timeouts ~label ~url __id
  in
