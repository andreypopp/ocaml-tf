...
let _ = fun (_ : okta_app_user_base_schema_property) -> ()

let yojson_of_okta_app_user_base_schema_property =
  (function
   | {
       app_id = v_app_id;
       id = v_id;
...
let _ = yojson_of_okta_app_user_base_schema_property

[@@@deriving.end]

let okta_app_user_base_schema_property ?id ?master ?pattern
    ?permissions ?required ?user_type ~app_id ~index ~title ~type_ ()
    : okta_app_user_base_schema_property =
  {
    app_id;
    id;
...
let make ?id ?master ?pattern ?permissions ?required ?user_type
    ~app_id ~index ~title ~type_ __id =
  let __type = "okta_app_user_base_schema_property" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_app_user_base_schema_property
        (okta_app_user_base_schema_property ?id ?master ?pattern
           ?permissions ?required ?user_type ~app_id ~index ~title
           ~type_ ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?master ?pattern ?permissions ?required
    ?user_type ~app_id ~index ~title ~type_ __id =
