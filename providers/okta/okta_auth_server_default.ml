...
let _ = fun (_ : okta_auth_server_default) -> ()

let yojson_of_okta_auth_server_default =
  (function
   | {
       ...
       in
       `Assoc bnds
    : okta_auth_server_default -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_okta_auth_server_default

[@@@deriving.end]

let okta_auth_server_default ?audiences ?credentials_rotation_mode
    ?description ?id ?issuer_mode ?name ?status () :
...
let make ?audiences ?credentials_rotation_mode ?description ?id
    ?issuer_mode ?name ?status __id =
  let __type = "okta_auth_server_default" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_auth_server_default
        (okta_auth_server_default ?audiences
           ?credentials_rotation_mode ?description ?id ?issuer_mode
           ?name ?status ());
    attrs = __attrs;
  }

let register ?tf_module ?audiences ?credentials_rotation_mode
    ?description ?id ?issuer_mode ?name ?status __id =
  let (r : _ Tf_core.resource) =
    make ?audiences ?credentials_rotation_mode ?description ?id
      ?issuer_mode ?name ?status __id
  in
