...
let _ = fun (_ : okta_group_schema_property) -> ()

let yojson_of_okta_group_schema_property =
  (function
   | {
       ...
       in
       `Assoc bnds
    : okta_group_schema_property -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_okta_group_schema_property
...
let okta_group_schema_property ?array_enum ?array_type ?description
    ?enum ?external_name ?external_namespace ?id ?master ?max_length
    ?min_length ?permissions ?required ?scope ?unique
...
let make ?array_enum ?array_type ?description ?enum ?external_name
    ...
    ?permissions ?required ?scope ?unique ?(array_one_of = [])
    ?(master_override_priority = []) ?(one_of = []) ~index ~title
    ~type_ __id =
  let __type = "okta_group_schema_property" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_group_schema_property
        (okta_group_schema_property ?array_enum ?array_type
           ?description ?enum ?external_name ?external_namespace ?id
           ?master ?max_length ?min_length ?permissions ?required
           ?scope ?unique ~array_one_of ~master_override_priority
           ~one_of ~index ~title ~type_ ());
    attrs = __attrs;
  }
...
let register ?tf_module ?array_enum ?array_type ?description ?enum
  ...
  let (r : _ Tf_core.resource) =
    make ?array_enum ?array_type ?description ?enum ?external_name
      ?external_namespace ?id ?master ?max_length ?min_length
      ?permissions ?required ?scope ?unique ~array_one_of
      ~master_override_priority ~one_of ~index ~title ~type_ __id
  in
