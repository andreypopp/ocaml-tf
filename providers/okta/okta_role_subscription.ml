...
let okta_role_subscription ?id ?status ~notification_type ~role_type
    () : okta_role_subscription =
  { id; notification_type; role_type; status }
...
let make ?id ?status ~notification_type ~role_type __id =
  ...
  in
  {
    ...
    json =
      yojson_of_okta_role_subscription
        (okta_role_subscription ?id ?status ~notification_type
           ~role_type ());
    attrs = __attrs;
  }

let register ?tf_module ?id ?status ~notification_type ~role_type
    __id =
  let (r : _ Tf_core.resource) =
    make ?id ?status ~notification_type ~role_type __id
  in
