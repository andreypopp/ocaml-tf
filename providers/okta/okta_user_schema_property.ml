...
let _ = fun (_ : master_override_priority) -> ()

let yojson_of_master_override_priority =
  (function
   | { type_ = v_type_; value = v_value } ->
       ...
       in
       `Assoc bnds
    : master_override_priority -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_master_override_priority
...
let _ = fun (_ : okta_user_schema_property) -> ()

let yojson_of_okta_user_schema_property =
  (function
   | {
       ...
       master_override_priority = v_master_override_priority;
       one_of = v_one_of;
     } ->
       let bnds : (string * Ppx_yojson_conv_lib.Yojson.Safe.t) list =
         []
       ...
       in
       let bnds =
         if Stdlib.( = ) [] v_master_override_priority then bnds
         else
           let arg =
             (yojson_of_list yojson_of_master_override_priority)
               v_master_override_priority
           in
           let bnd = "master_override_priority", arg in
           bnd :: bnds
       in
       let bnds =
       ...
       in
       `Assoc bnds
    : okta_user_schema_property -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_okta_user_schema_property

[@@@deriving.end]

let array_one_of ~const ~title () : array_one_of = { const; title }

let master_override_priority ?type_ ~value () :
    master_override_priority =
  { type_; value }

let one_of ~const ~title () : one_of = { const; title }

let okta_user_schema_property ?array_enum ?array_type ?description
    ?enum ?external_name ?external_namespace ?id ?master ?max_length
    ?min_length ?pattern ?permissions ?required ?scope ?unique
    ?user_type ?(array_one_of = []) ?(master_override_priority = [])
    ?(one_of = []) ~index ~title ~type_ () :
...
let make ?array_enum ?array_type ?description ?enum ?external_name
    ?external_namespace ?id ?master ?max_length ?min_length ?pattern
    ?permissions ?required ?scope ?unique ?user_type
    ?(array_one_of = []) ?(master_override_priority = [])
    ?(one_of = []) ~index ~title ~type_ __id =
  let __type = "okta_user_schema_property" in
  let __attrs =
    ({
       tf_name = __id;
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_user_schema_property
        (okta_user_schema_property ?array_enum ?array_type
           ?description ?enum ?external_name ?external_namespace ?id
           ?master ?max_length ?min_length ?pattern ?permissions
           ?required ?scope ?unique ?user_type ~array_one_of
           ~master_override_priority ~one_of ~index ~title ~type_ ());
    attrs = __attrs;
  }
...
let register ?tf_module ?array_enum ?array_type ?description ?enum
  ...
  let (r : _ Tf_core.resource) =
    make ?array_enum ?array_type ?description ?enum ?external_name
      ...
      ?pattern ?permissions ?required ?scope ?unique ?user_type
      ~array_one_of ~master_override_priority ~one_of ~index ~title
      ~type_ __id
  in
