...
let _ = fun (_ : okta_app_basic_auth) -> ()

let yojson_of_okta_app_basic_auth =
  (function
   | {
       ...
       in
       let bnds =
         let arg = yojson_of_prop yojson_of_string v_auth_url in
         ("auth_url", arg) :: bnds
       in
       let bnds =
       ...
       in
       `Assoc bnds
    : okta_app_basic_auth -> Ppx_yojson_conv_lib.Yojson.Safe.t)

let _ = yojson_of_okta_app_basic_auth
...
let okta_app_basic_auth ?accessibility_error_redirect_url
    ?accessibility_login_redirect_url ?accessibility_self_service
    ?admin_note ?app_links_json ?auto_submit_toolbar ?enduser_note
    ?hide_ios ?hide_web ?id ?logo ?status ?timeouts ~auth_url ~label
    ~url () : okta_app_basic_auth =
  {
    accessibility_error_redirect_url;
    accessibility_login_redirect_url;
...
let make ?accessibility_error_redirect_url
    ...
    ?hide_ios ?hide_web ?id ?logo ?status ?timeouts ~auth_url ~label
    ~url __id =
  let __type = "okta_app_basic_auth" in
  let __attrs =
    ({
       tf_name = __id;
       accessibility_error_redirect_url =
  ...
  in
  {
    ...
    type_ = __type;
    json =
      yojson_of_okta_app_basic_auth
        (okta_app_basic_auth ?accessibility_error_redirect_url
           ?accessibility_login_redirect_url
           ?accessibility_self_service ?admin_note ?app_links_json
           ?auto_submit_toolbar ?enduser_note ?hide_ios ?hide_web ?id
           ?logo ?status ?timeouts ~auth_url ~label ~url ());
    attrs = __attrs;
  }
...
let register ?tf_module ?accessibility_error_redirect_url
  ...
  let (r : _ Tf_core.resource) =
    make ?accessibility_error_redirect_url
      ?accessibility_login_redirect_url ?accessibility_self_service
      ?admin_note ?app_links_json ?auto_submit_toolbar ?enduser_note
      ?hide_ios ?hide_web ?id ?logo ?status ?timeouts ~auth_url
      ~label ~url __id
  in
